...
class NodeViewModel: NSObject {
  ...
  private func getSection() -> SettingSectionModel {
    ...
    home.rowHeight = 44.0
    home.switchOpen = IMKitClient.instance.getSettingRepo().getNodeValue() == true ? true : false

    // 海外节点配置
    let overseas = SettingCellModel()
    overseas.subTitle = NSLocalizedString("overseas_node", comment: "")
    overseas.switchOpen = IMKitClient.instance.getSettingRepo().getNodeValue() == true ? false : true
