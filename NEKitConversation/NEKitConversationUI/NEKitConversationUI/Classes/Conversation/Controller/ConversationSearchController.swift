...
import NIMSDK
 ...
 class SearchSessionHeaderView:UITableViewHeaderFooterView {
     ...
     override init(reuseIdentifier: String?) {
         ...
         NSLayoutConstraint.activate([
             bottomLine.rightAnchor.constraint(equalTo: self.rightAnchor,constant: -20),
...
public class ConversationSearchController: NEBaseViewController {
        ...
        if searchText.count <= 0 {
         ...
         if textRange == nil || ((textRange?.isEmpty) == nil) {
             ...
             viewModel.doSearch(searchStr: searchText) { error, tupleInfo in
                 if let err = error {
                     QChatLog.errorLog(self.tag, desc: "❌doSearch failed,error = \(err)")
                 }else {
                     if tupleInfo?.friend.count == 0 && tupleInfo?.contactGroup.count == 0 && tupleInfo?.seniorGroup.count == 0{
    ...
    private lazy var tableView:UITableView = {
        ...
        tableView.register(ConversationSearchCell.self, forCellReuseIdentifier: "\(NSStringFromClass(ConversationSearchCell.self))")
        tableView.rowHeight = 60
        tableView.backgroundColor = .white
        tableView.register(SearchSessionHeaderView.self, forHeaderFooterViewReuseIdentifier: "\(NSStringFromClass(SearchSessionHeaderView.self))")
...
extension ConversationSearchController:UITableViewDelegate,UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "\(NSStringFromClass(ConversationSearchCell.self))", for: indexPath) as! ConversationSearchCell
    ...
    public func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let sectionView = tableView.dequeueReusableHeaderFooterView(withIdentifier: "\(NSStringFromClass(SearchSessionHeaderView.self))") as! SearchSessionHeaderView
        sectionView.setUpTitle(title: headTitleArr[section])
        sectionView.backgroundView = UIView.init()
        return sectionView
    }
    ...
    public func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        }else if let contactGroup = viewModel.searchResult?.contactGroup,contactGroup.count > 0, section == 1 {
            return 30
        }else if let seniorGroup = viewModel.searchResult?.seniorGroup,seniorGroup.count > 0,section == 2 {
            return 30
        }else {
            return 0
        }
