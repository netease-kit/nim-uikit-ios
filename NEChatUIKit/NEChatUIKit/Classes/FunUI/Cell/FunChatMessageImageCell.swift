...
open class FunChatMessageImageCell: FunChatMessageBaseCell {
  public lazy var contentImageViewLeft: UIImageView = {
    ...
    imageView.image = coreLoader.loadImage("fun_default_image")
  ...
  override open func setModel(_ model: MessageContentModel, _ isSend: Bool) {
    ...
    if let m = model as? MessageImageModel, let imageUrl = m.urlString {
        ...
        contentImageView.sd_setImage(with: url) { image, error, type, url in
          if let err = error as? NSError {
            if err.code == 2001,
              ...
              contentImageView.image = coreLoader.loadImage("default_image")
            }
          }
        }
      } else {
      ...
      contentImageView.image = coreLoader.loadImage("fun_default_image")
    }
  }
}
