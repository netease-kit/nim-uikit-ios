...
open class NEBaseReadViewController: NEChatBaseViewController, UITableViewDelegate,
  ...
  /// 已读/未读 按钮下方横线
  lazy var bottonBottomLine: UIView = {
    let view = UIView()
  ...
  /// 已读 tableView
  lazy var readTableView: UITableView = {
  ...
  /// 已读 tableView
  lazy var unreadTableView: UITableView = {
    let tableView = UITableView(frame: .zero, style: .plain)
    tableView.delegate = self
    tableView.dataSource = self
    tableView.sectionHeaderHeight = 0
  ...
  open func commonUI() {
    ...
    if #available(iOS 11.0, *) {
      NSLayoutConstraint.activate([
        ...
        readTableView.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor),
      ])
    } else {
      NSLayoutConstraint.activate([
        readTableView.topAnchor.constraint(equalTo: readButton.bottomAnchor, constant: 1),
    ...
    if #available(iOS 11.0, *) {
      NSLayoutConstraint.activate([
        ...
        unreadTableView.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor),
      ])
    } else {
      NSLayoutConstraint.activate([
        unreadTableView.topAnchor.constraint(equalTo: readButton.bottomAnchor, constant: 1),
