...
public class ChatMessageHelper: NSObject {
  ...
  public static func loadKeywordInMessage(_ message: V2NIMMessage?,
                                          ...
                                          _ keyword: String,
                                          _ themeColor: UIColor) -> NSMutableAttributedString? {
    let text = attributeStr?.string ?? message?.text ?? ""
    let messageTextSize = message?.isSelf == true ? ChatUIConfig.shared.messageProperties.selfMessageTextSize : ChatUIConfig.shared.messageProperties.receiveMessageTextSize
    let messageTextFont = UIFont.systemFont(ofSize: messageTextSize)
    let messageTextColor = message?.isSelf == true ? ChatUIConfig.shared.messageProperties.selfMessageTextColor : ChatUIConfig.shared.messageProperties.receiveMessageTextColor
    ...
    guard let matches = matches else { return nil }

    for match in matches {
      let modelStart = match.range.location
      let modelEnd = match.range.location + match.range.length

      // 前面因为表情增加的索引数量
      var count = 0
      if modelStart > 0, text.count > modelStart {
        let frontAttributeStr = NEEmotionTool.getAttWithStr(
          str: String(text.prefix(modelStart)),
          font: messageTextFont,
          color: messageTextColor
        )
        count = getReduceIndexCount(frontAttributeStr)
      }
      let start = modelStart - count
      if start < 0 {
        notFound = true
        break
      }
      var end = modelEnd - count

      if modelEnd > text.count {
        notFound = true
        break
      }
      // 获取起始索引
      let startIndex = text.index(text.startIndex, offsetBy: modelStart)
      // 获取结束索引
      let endIndex = text.index(text.startIndex, offsetBy: modelEnd)
      let frontAttributeStr = NEEmotionTool.getAttWithStr(
        str: String(text[startIndex ..< endIndex]),
        font: messageTextFont,
        color: messageTextColor
      ...
      if attributeStr?.length ?? 0 >= end {
        attributeStr?.addAttribute(NSAttributedString.Key.foregroundColor, value: themeColor, range: NSMakeRange(start, end - start))
      }
    }
    ...
    if notFound {
      for match in matches {
        ...
        if attributeStr?.length ?? 0 >= modelEnd {
          attributeStr?.addAttribute(NSAttributedString.Key.foregroundColor, value: themeColor, range: NSMakeRange(modelStart, modelEnd - modelStart))
        }
      }
    }
