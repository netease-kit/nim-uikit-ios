...
public class SelectUserViewController: NEBaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func commonUI() {
        ...
        tableView.register(ChatTeamMemberCell.self, forCellReuseIdentifier: "\(ChatTeamMemberCell.self)")
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            if let model = self.teamInfo?.users[indexPath.row - 1] {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let block = self.selectedBlock {
            block(indexPath.row, self.teamInfo?.users[indexPath.row - 1])
        }
        self.dismiss(animated: true, completion: nil)
    }
