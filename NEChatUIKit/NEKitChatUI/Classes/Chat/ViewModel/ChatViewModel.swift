                ...
                if let messageArray = messages, messageArray.count > 0 {
                    ...
                    if let updateMessage = messageArray.first,updateCredible {
                        ...
                        weakSelf?.repo.updateIncompleteSessions(messages: [updateMessage], { error, recentSessions in
                            if error != nil {
                                NELog.errorLog(weakSelf?.className ?? "ChatViewModel", desc: "❌updateIncompleteSessions failed，error = \(error!)")
                            }
                        })
                    }
...
//        mark session read
        ...
        repo.markReadInSession(session) { error in
            if error != nil {
                NELog.errorLog(weakself?.className() ?? "ChatViewModel", desc: "❌markReadInSession failed,error = \(error!)")
            }
        }
    }
