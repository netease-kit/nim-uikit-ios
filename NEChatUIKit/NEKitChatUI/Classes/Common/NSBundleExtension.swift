...
public extension Bundle {
    ...
    class func nim_EmojiPlistFile() -> String? {
        ...
        let filepath = bundle?.path(forResource: "emoji_ios_new", ofType: "plist", inDirectory: NIMKit_EmojiPath)
        return filepath
    }
    ...
    class func nim_EmojiImage(imageName:String) -> String? {
        ...
        if UIScreen.main.scale == 3.0 {
            path = bundle?.path(forResource: tribleImage, ofType: ext, inDirectory: NIMKit_EmojiPath)
        }
        if let imagePath = path,imagePath.count>0 {
            
        }else {
            path = bundle?.path(forResource: doubleImage, ofType: ext, inDirectory: NIMKit_EmojiPath)
        }
        return path
    }
