...
@interface UIView (NIMKitToastPrivate)

- (void)nimkit_hideToast:(UIView *)toast;
- (void)nimkit_toastTimerDidFinish:(NSTimer *)timer;
- (void)nimkit_handleToastTapped:(UITapGestureRecognizer *)recognizer;
- (CGPoint)nimkit_centerPointForPosition:(id)position withToast:(UIView *)toast;
- (UIView *)nimkit_viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image;
- (CGSize)nimkit_sizeForString:(NSString *)string font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineBreakMode:(NSLineBreakMode)lineBreakMode;

@end


@implementation UIView (NIMKitToast)
...
- (void)nimkit_makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {
    UIView *toast = [self nimkit_viewForMessage:message title:nil image:nil];
    [self nimkit_showToast:toast duration:duration position:position];
}

- (void)nimkit_makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title {
    UIView *toast = [self nimkit_viewForMessage:message title:title image:nil];
    [self nimkit_showToast:toast duration:duration position:position];
}

- (void)nimkit_makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position image:(UIImage *)image {
    UIView *toast = [self nimkit_viewForMessage:message title:nil image:image];
    [self nimkit_showToast:toast duration:duration position:position];  
}

- (void)nimkit_makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(id)position title:(NSString *)title image:(UIImage *)image {
    UIView *toast = [self nimkit_viewForMessage:message title:title image:image];
    [self nimkit_showToast:toast duration:duration position:position];
}

- (void)nimkit_showToast:(UIView *)toast {
    [self nimkit_showToast:toast duration:NIMKitToastDefaultDuration position:nil];
}


- (void)nimkit_showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position {
    [self nimkit_showToast:toast duration:duration position:position tapCallback:nil];
...
{
    toast.center = [self nimkit_centerPointForPosition:position withToast:toast];
    toast.alpha = 0.0;
    
    if (NIMKitToastHidesOnTap) {
        UITapGestureRecognizer *recognizer = [[UITapGestureRecognizer alloc] initWithTarget:toast action:@selector(nimkit_handleToastTapped:)];
    ...
    [UIView animateWithDuration:NIMKitToastFadeDuration
                     ...
                     animations:^{
                         ...
                         NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:duration target:self selector:@selector(nimkit_toastTimerDidFinish:) userInfo:toast repeats:NO];
...
- (void)nimkit_toastTimerDidFinish:(NSTimer *)timer {
    [self nimkit_hideToast:(UIView *)timer.userInfo];
}
...
- (void)nimkit_handleToastTapped:(UITapGestureRecognizer *)recognizer {
    ...
    [self nimkit_hideToast:recognizer.view];
}

#pragma mark - Toast Activity Methods

- (void)nimkit_makeToastActivity {
    [self nimkit_makeToastActivity:NIMKitToastActivityDefaultPosition];
}
...
- (void)nimkit_makeToastActivity:(id)position {
    ...
    activityView.center = [self nimkit_centerPointForPosition:position withToast:activityView];
...
- (CGPoint)nimkit_centerPointForPosition:(id)point withToast:(UIView *)toast {
...
- (UIView *)nimkit_viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (title != nil) {
        ...
        CGSize expectedSizeTitle = [self nimkit_sizeForString:title font:titleLabel.font constrainedToSize:maxSizeTitle lineBreakMode:titleLabel.lineBreakMode];
    ...
    if (message != nil) {
        ...
        CGSize expectedSizeMessage = [self nimkit_sizeForString:message font:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
