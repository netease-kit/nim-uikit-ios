...
//    }()
    ...
    func setupUI(){
        ...
        let clearItem = UIBarButtonItem(title: "保存", style: .done, target: self, action: #selector(saveAlias))
        clearItem.tintColor = UIColor(hexString: "337EFF")
        navigationItem.rightBarButtonItem = clearItem
        self.title = localizable("备注名")
...
//        }
        ...
        if let u = user {
            ...
            viewmodel.update(u) { error in
                ...
                if let err = error {
                    weakSelf?.view.makeToast(err.localizedDescription, duration: 2, position: .center)
                }else {
