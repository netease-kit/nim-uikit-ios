...
public class ValidationMessageViewController: ContactBaseViewController {
    ...
    func setupUI(){
        ...
        tableView.register(SystemNotificationCell.self, forCellReuseIdentifier: "\(SystemNotificationCell.self)")
    }
...
extension ValidationMessageViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: reuseIdentifier, for: indexPath) as! SystemNotificationCell
        cell.delegate = self
        cell.confige(noti)
        return cell
    }
    ...
    func onAccept(_ notifiModel: XNotification) {
        ...
        if notifiModel.type == .teamInvite {
            viewModel.acceptInviteWithTeam(teamId, invitorId) { error in
                if error != nil {
                    QChatLog.infoLog(self.tag, desc: "❌acceptInviteWithTeam failed,error = \(error!)")
                }else {
            ...
            viewModel.agreeRequest(invitorId) { error in
                if error != nil {
                    QChatLog.infoLog(self.tag, desc: "❌agreeRequest failed,error = \(error!)")
                }else {
                    notifiModel.handleStatus = .HandleTypeOk
                    notifiModel.imNotification?.handleStatus = 1
                    weakSelf?.tableView.reloadData()
    ...
    func onRefuse(_ notifiModel: XNotification) {
        ...
        if notifiModel.type == .teamInvite {
            weakSelf?.viewModel.rejectInviteWithTeam(teamId, invitorId, { error in
                if error != nil {
                    QChatLog.infoLog(self.tag, desc: "❌rejectInviteWithTeam failed,error = \(error!)")
                }else {
