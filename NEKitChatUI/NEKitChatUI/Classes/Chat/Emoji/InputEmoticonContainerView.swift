...
public class InputEmoticonContainerView: UIView {
    ...
    private var currentCatalogData:NIMInputEmoticonCatalog? {
        set {
            ...
            if let currentData = newValue {
                emoticonPageView.scrollToPage(page: pageIndexWithEmoticon(emoticonCatalog: currentData))
            }
        }
        get {
            return _currentCatalogData
        }
    }
    ...
    func setUpSubViews(){
        self.addSubview(emoticonPageView)
        self.addSubview(tabView)
        
        NSLayoutConstraint.activate([
            emoticonPageView.topAnchor.constraint(equalTo: self.topAnchor),
            emoticonPageView.rightAnchor.constraint(equalTo: self.rightAnchor),
            emoticonPageView.leftAnchor.constraint(equalTo: self.leftAnchor),
            emoticonPageView.heightAnchor.constraint(equalToConstant: 159)
        ])
        
        NSLayoutConstraint.activate([
            tabView.bottomAnchor.constraint(equalTo: self.bottomAnchor),
            tabView.rightAnchor.constraint(equalTo: self.rightAnchor),
            tabView.leftAnchor.constraint(equalTo: self.leftAnchor),
    ...
    func loadDefaultCatalog() -> NIMInputEmoticonCatalog?{
        let emoticonCatalog = NIMInputEmoticonManager.shared.emoticonCatalog(catalogID: NIMKit_EmojiCatalog)
        let layout = NIMInputEmoticonLayout.init(width: self.width)
    ...
    func numberOfPagesWithEmoticon(emoticonCatalog:NIMInputEmoticonCatalog?) -> NSInteger {
        ...
        if let emotionsCount = emoticonCatalog?.emoticons?.count,let layoutCount =  emoticonCatalog?.layout?.itemCountInPage{
            ...
            QChatLog.errorLog(classTag, desc: "❌count maybe nil")
            return 0
        }
    }
    ...
    private lazy var tabView:InputEmoticonTabView = {
        let pageView = InputEmoticonTabView.init(frame:CGRect.zero)
...
extension InputEmoticonContainerView {
    ...
    func emojiPageView(pageView:EmojiPageView,emoticon:NIMInputEmoticonCatalog,page:NSInteger) -> UIView {
        let subView = UIView()
        guard let layout = emoticon.layout else {
            QChatLog.errorLog(classTag, desc: "layout is nil")
            return UIView.init()
        }

        guard let emotions = emoticon.emoticons else {
            QChatLog.errorLog(classTag, desc: "emoticon.emoticons is nil")
            return UIView.init()
        }
        ...
        for i in (begin..<end) {
            ...
            if let id = emoticon.catalogID {
                let button = NIMInputEmoticonButton.iconButtonWithData(data: data, catalogID: id, delegate: self)
                ...
                let iconRect = CGRect.init(x: CGFloat(x), y: CGFloat(y), width: iconWidth, height: iconHeight)
        ...
        if emoticon.catalogID == NIMKit_EmojiCatalog {
            addDeleteEmotButtonToView(view: subView, coloumnIndex: coloumnIndex, rowIndex: rowIndex, startX: CGFloat(startX), startY: CGFloat(startY), iconWidth: iconWidth, iconHeight: iconHeight, emotion: emoticon)
        }
        return subView
    ...
    func addDeleteEmotButtonToView(view:UIView,coloumnIndex:NSInteger,rowIndex:NSInteger,startX:CGFloat,startY:CGFloat,iconWidth:CGFloat,iconHeight:CGFloat,emotion:NIMInputEmoticonCatalog) {
        
        guard let layout = emotion.layout else {
            QChatLog.errorLog(classTag, desc: "❌emotion is nill")
            return
        }
        ...
        let deleteIconRect = CGRect.init(x: newX, y: newY, width: NIMKit_DeleteIconWidth, height: NIMKit_DeleteIconHeight)
...
extension InputEmoticonContainerView:EmojiPageViewDelegate,EmojiPageViewDataSource {
    ...
    public func pageView(pageView: EmojiPageView?, index: NSInteger) -> UIView {
        ...
        guard let totalData = totalCatalogData,let targetView = pageView else {
            QChatLog.errorLog(classTag, desc: "❌totalCatalogData is nil")
            return UIView.init()
        }
...
extension InputEmoticonContainerView:NIMInputEmoticonButtonDelegate {
    ...
    public func selectedEmoticon(emotion: NIMInputEmoticon, catalogID: String) {
        ...
        guard let emotionId = emotion.emoticonID else {
            QChatLog.errorLog(classTag, desc: "❌emoticonID is nil")
            return
        }
        if emotion.type == .unicode {
            delegate?.selectedEmoticon(emoticonID: emotionId, emotCatalogID: catalogID, description: emotion.unicode ?? "")
        }else {
            delegate?.selectedEmoticon(emoticonID: emotionId, emotCatalogID: catalogID, description: emotion.tag ?? "")
