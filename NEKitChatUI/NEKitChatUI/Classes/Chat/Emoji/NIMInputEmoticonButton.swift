...
public class NIMInputEmoticonButton: UIButton {
    ...
    class public func iconButtonWithData(data:NIMInputEmoticon,catalogID:String,delegate:NIMInputEmoticonButtonDelegate) -> NIMInputEmoticonButton{
        let icon = NIMInputEmoticonButton.init()
    ...
    @objc func onIconSelected(sender:NIMInputEmoticonButton) {
        ...
        guard let data = emotionData,let id = catalogID else {
            QChatLog.errorLog(classsTag, desc: "emotionData or catalogID maybe nil")
            return
        }
