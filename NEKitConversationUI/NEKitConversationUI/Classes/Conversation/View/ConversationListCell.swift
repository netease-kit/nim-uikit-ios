...
open class ConversationListCell: UITableViewCell {
    ...
    func setupSubviews(){
        ...
        self.contentView.addSubview(notifyMsg)
        
        NSLayoutConstraint.activate([
            headImge.leftAnchor.constraint(equalTo: self.contentView.leftAnchor,constant: NEConstant.screenInterval),
        ...
        NSLayoutConstraint.activate([
            timeLabel.rightAnchor.constraint(equalTo: self.contentView.rightAnchor,constant: -NEConstant.screenInterval),
            timeLabel.topAnchor.constraint(equalTo: self.contentView.topAnchor,constant: 17),
        ])
        ...
        NSLayoutConstraint.activate([
            ...
            subTitle.rightAnchor.constraint(equalTo: self.contentView.rightAnchor,constant: -50),
            subTitle.topAnchor.constraint(equalTo: title.bottomAnchor,constant: 6)
        ])
        
        NSLayoutConstraint.activate([
            notifyMsg.rightAnchor.constraint(equalTo: self.timeLabel.rightAnchor),
            notifyMsg.topAnchor.constraint(equalTo: self.timeLabel.bottomAnchor,constant: 5),
     ...
     func configData(sessionModel:ConversationListModel?){
        ...
        if conversationModel.recentSession?.session?.sessionType == .P2P {
            ...
            if let imageName = conversationModel.userInfo?.userInfo?.avatarUrl {
                headImge.setTitle("")
                headImge.sd_setImage(with: URL.init(string: imageName), completed: nil)
            }else {
                headImge.setTitle(conversationModel.userInfo?.showName() ?? "")
                headImge.sd_setImage(with:nil, completed: nil)
                headImge.backgroundColor = UIColor.colorWithString(string: conversationModel.userInfo?.userId)
            }
            ...
            //notifyForNewMsg
            notifyMsg.isHidden = viewModel.notifyForNewMsg(userId: conversationModel.userInfo?.userId)
            ...
            if let imageName = conversationModel.teamInfo?.avatarUrl {
                ...
                headImge.sd_setImage(with:nil, completed: nil)
                headImge.backgroundColor = UIColor.colorWithString(string: conversationModel.teamInfo?.teamId)
            }
            title.text = conversationModel.teamInfo?.getShowName()
            
            //notifyForNewMsg
            let teamNotifyState = viewModel.notifyStateForNewMsg(teamId: conversationModel.teamInfo?.teamId)
        ...
        if let rencentSession = conversationModel.recentSession {
            timeLabel.text = dealTime(time: timestampDescriptionForRecentSession(recentSession: rencentSession))
        }
       ...
       //backgroundColor
        if let session = conversationModel.recentSession?.session{
            ...
            if isTop {
                self.contentView.backgroundColor = UIColor.init(hexString: "0xF3F5F7")
            }else {
