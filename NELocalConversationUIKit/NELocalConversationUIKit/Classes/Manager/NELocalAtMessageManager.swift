...
open class NELocalAtMessageManager: NSObject, NEIMKitClientListener, NEChatListener {
  ...
  private func addAtRecordWithCompare(message: V2NIMMessage, record: inout [String: LocalAtMEMessageRecord]) {
    ...
    LocalConversationRepo.shared.getConversationReadTime(conversationId) { timestap, error in
      if error != nil {
        NEALog.infoLog(NELocalAtMessageManager.className(), desc: #function + "getConversationReadTime error : \(error?.localizedDescription ?? "")")
      }

      if let t = timestap {
        compareTimestap = t
        NEALog.infoLog(NELocalAtMessageManager.className(), desc: #function + "getConversationReadTime time \(t)")
      }
      semaphore.signal()
    }
