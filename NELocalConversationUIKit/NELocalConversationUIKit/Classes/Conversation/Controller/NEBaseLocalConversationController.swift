...
extension NEBaseLocalConversationController: TabNavigationViewDelegate {
  ...
  open func openScanQR() {
    ...
    // 返回 true 表示已处理（退出扫码页），false 表示无效（继续扫描）
  ...
  open func getScanQRViewController() -> NEBaseLocalScanQRViewController {
    NEBaseLocalScanQRViewController()
  }
  ...
  open func handleQRScanResult(_ result: String) -> Bool {
    ...
    if let data = result.data(using: .utf8),
      ...
      navigationController?.transitionCoordinator?.animate(alongsideTransition: nil) { [weak self] _ in
        ...
        viewControllers.removeAll { $0 is NEBaseLocalScanQRViewController }
