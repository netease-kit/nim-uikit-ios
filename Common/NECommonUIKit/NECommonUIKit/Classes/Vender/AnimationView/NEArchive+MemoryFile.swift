...
extension NEArchive {
  ...
  class NEMemoryFile {
    ...
    func open(mode: String) -> NEFILEPointer? {
  ...
  fileprivate extension NEMemoryFile {
  ...
  private func fileFromCookie(cookie: UnsafeRawPointer) -> NEMemoryFile {
    Unmanaged<NEMemoryFile>.fromOpaque(cookie).takeUnretainedValue()
  }

  private func closeStub(_ cookie: UnsafeMutableRawPointer?) -> Int32 {
    if let cookie {
      Unmanaged<NEMemoryFile>.fromOpaque(cookie).release()
    }
    return 0
  }
