...
struct NECGLine {
  ...
  func normalize() -> NECGLine {
    ...
    return NECGLine(start: start, end: absoluteVector)
  }

  /// Trims a line segment to the given length
  func trimmedToLength(_ toLength: CGFloat) -> NECGLine {
    let len = length
    guard len > 0 else {
      return self
    }
  ...
  func flipped() -> NECGLine {
    let relativeEnd = end - start
    let flippedEnd = CGPoint(x: relativeEnd.x * -1, y: relativeEnd.y * -1)
    return NECGLine(start: start, end: flippedEnd + start)
  }

  /// Move the line to the new start point.
  func transpose(_ toPoint: CGPoint) -> NECGLine {
    let diff = toPoint - start
    let newEnd = end + diff
    return NECGLine(start: toPoint, end: newEnd)
  }
}
