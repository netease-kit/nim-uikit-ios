...
extension FileManager {
  ...
  class func attributes(from entry: NEEntry) -> [FileAttributeKey: Any] {
    ...
    guard let osType = NEEntry.OSType(rawValue: UInt(versionMadeBy >> 8)) else { return attributes }
  ...
  class func permissions(for externalFileAttributes: UInt32,
                         osType: NEEntry.OSType,
                         entryType: NEEntry.NEEntryType)
    -> UInt16 {
    switch osType {
    case .unix, .osx:
  ...
  class func externalFileAttributesForEntry(of type: NEEntry.NEEntryType, permissions: UInt16) -> UInt32 {
    var typeInt: UInt16
    switch type {
    case .file:
  ...
  class func typeForItem(at url: URL) throws -> NEEntry.NEEntryType {
    let fileManager = FileManager()
    ...
    var fileStat = stat()
    lstat(entryFileSystemRepresentation, &fileStat)
    return NEEntry.NEEntryType(mode: mode_t(fileStat.st_mode))
  }
  ...
  /// represented within the ZIP `NEArchive`. Calling this method with a directory URL
  ...
  ///   - compressionMethod: Indicates the `NECompressionMethod` that should be applied.
  ...
  func zipItem(at sourceURL: URL,
    ...
    guard let archive = NEArchive(url: destinationURL, accessMode: .create) else {
      throw NEArchive.NEArchiveError.unwritableArchive
    }
    ...
    if isDirectory {
      ...
      // as common base for all entries (similar to macOS' NEArchive Utility.app)
  ...
  func unzipItem(at sourceURL: URL,
    ...
    guard let archive = NEArchive(url: sourceURL, accessMode: .read, preferredEncoding: preferredEncoding) else {
      throw NEArchive.NEArchiveError.unreadableArchive
    }
    ...
    for entry in sortedEntries {
      ...
      func verifyChecksumIfNecessary() throws {
        if skipCRC32 == false, crc32 != entry.checksum {
          throw NEArchive.NEArchiveError.invalidCRC32
        }
      }
