...
final class NETextAnimator: Codable, NEDictionaryInitializable {
  // MARK: Lifecycle

  required init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: NETextAnimator.NECodingKeys.self)
    name = try container.decodeIfPresent(String.self, forKey: .name) ?? ""
    let animatorContainer = try container.nestedContainer(keyedBy: TextAnimatorKeys.self, forKey: .textAnimator)
    fillColor = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieColor>.self, forKey: .fillColor)
    strokeColor = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieColor>.self, forKey: .strokeColor)
    strokeWidth = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .strokeWidth)
    tracking = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .tracking)
    anchor = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector3D>.self, forKey: .anchor)
    position = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector3D>.self, forKey: .position)
    scale = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector3D>.self, forKey: .scale)
    skew = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .skew)
    skewAxis = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .skewAxis)
    rotationX = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .rotationX)
    rotationY = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .rotationY)
    if let rotation = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .rotation) {
      rotationZ = rotation
    } else if let rotation = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .rotationZ) {
      rotationZ = rotation
    } else {
      rotationZ = nil
    }
    opacity = try animatorContainer.decodeIfPresent(NEKeyframeGroup<NELottieVector1D>.self, forKey: .opacity)
  }

  init(dictionary: [String: Any]) throws {
    name = (try? dictionary.value(for: NECodingKeys.name)) ?? ""
    let animatorDictionary: [String: Any] = try dictionary.value(for: NECodingKeys.textAnimator)
    if let fillColorDictionary = animatorDictionary[TextAnimatorKeys.fillColor.rawValue] as? [String: Any] {
      fillColor = try? NEKeyframeGroup<NELottieColor>(dictionary: fillColorDictionary)
    } else {
      fillColor = nil
    }
    if let strokeColorDictionary = animatorDictionary[TextAnimatorKeys.strokeColor.rawValue] as? [String: Any] {
      strokeColor = try? NEKeyframeGroup<NELottieColor>(dictionary: strokeColorDictionary)
    } else {
      strokeColor = nil
    }
    if let strokeWidthDictionary = animatorDictionary[TextAnimatorKeys.strokeWidth.rawValue] as? [String: Any] {
      strokeWidth = try? NEKeyframeGroup<NELottieVector1D>(dictionary: strokeWidthDictionary)
    } else {
      strokeWidth = nil
    }
    if let trackingDictionary = animatorDictionary[TextAnimatorKeys.tracking.rawValue] as? [String: Any] {
      tracking = try? NEKeyframeGroup<NELottieVector1D>(dictionary: trackingDictionary)
    } else {
      tracking = nil
    }
    if let anchorDictionary = animatorDictionary[TextAnimatorKeys.anchor.rawValue] as? [String: Any] {
      anchor = try? NEKeyframeGroup<NELottieVector3D>(dictionary: anchorDictionary)
    } else {
      anchor = nil
    }
    ...
    if let skewDictionary = animatorDictionary[TextAnimatorKeys.skew.rawValue] as? [String: Any] {
      skew = try? NEKeyframeGroup<NELottieVector1D>(dictionary: skewDictionary)
    } else {
      skew = nil
    }
    if let skewAxisDictionary = animatorDictionary[TextAnimatorKeys.skewAxis.rawValue] as? [String: Any] {
      skewAxis = try? NEKeyframeGroup<NELottieVector1D>(dictionary: skewAxisDictionary)
    } else {
      skewAxis = nil
    }
  ...
  /// NEStroke Color
  let strokeColor: NEKeyframeGroup<NELottieColor>?

  /// NEFill Color
  let fillColor: NEKeyframeGroup<NELottieColor>?

  /// NEStroke Width
  let strokeWidth: NEKeyframeGroup<NELottieVector1D>?

  /// Tracking
  let tracking: NEKeyframeGroup<NELottieVector1D>?
