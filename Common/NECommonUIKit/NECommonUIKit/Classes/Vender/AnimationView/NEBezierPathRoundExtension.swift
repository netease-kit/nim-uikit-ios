...
extension NECompoundBezierPath {
  // Round corners of a compound bezier
  func roundCorners(radius: CGFloat) -> NECompoundBezierPath {
...
extension NEBezierPath {
  // Computes a new `NEBezierPath` with each corner rounded based on the given `radius`
  func roundCorners(radius: CGFloat) -> NEBezierPath {
    var newPath = NEBezierPath()
    var uniquePath = NEBezierPath()

    var currentVertex: NECurveVertex
    var closestVertex: NECurveVertex
