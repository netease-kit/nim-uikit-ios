...
extension NEKeypathSearchable {
  func animatorNodes(for keyPath: NEAnimationKeypath) -> [NEAnimatorNode]? {
    ...
    guard let nextKeypath = keyPath.popKey(keypathName) else {
      ...
      if
        let node = self as? NEAnimatorNode,
  ...
  func nodeProperties(for keyPath: NEAnimationKeypath) -> [NEAnyNodeProperty]? {
    guard let nextKeypath = keyPath.popKey(keypathName) else {
      /// Nope. Stop Search
      return nil
    }
  ...
  func keypath(for layer: CALayer) -> NEAnimationKeypath? {
  ...
  func allKeypaths(for keyPath: NEAnimationKeypath? = nil) -> [String] {
    var allKeypaths: [String] = []

    let newKeypath: NEAnimationKeypath
    if let previousKeypath = keyPath {
      newKeypath = previousKeypath.appendingKey(keypathName)
    } else {
      newKeypath = NEAnimationKeypath(keys: [keypathName])
    }
  ...
  /// Computes the list of animation keypaths that descend from this layer
  func layerKeypaths(for keyPath: NEAnimationKeypath? = nil) -> [CALayer: NEAnimationKeypath] {
    var allKeypaths: [CALayer: NEAnimationKeypath] = [:]
...
extension NEAnimationKeypath {
  ...
  func popKey(_ keyname: String) -> NEAnimationKeypath? {
    guard
      let currentKey,
    ...
    return NEAnimationKeypath(keys: newKeys)
  }

  func appendingKey(_ key: String) -> NEAnimationKeypath {
    var newKeys = keys
    newKeys.append(key)
    return NEAnimationKeypath(keys: newKeys)
  }
