...
struct NECurveVertex {
  ...
  func reversed() -> NECurveVertex {
    NECurveVertex(point: point, inTangent: outTangent, outTangent: inTangent)
  }

  func translated(_ translation: CGPoint) -> NECurveVertex {
    NECurveVertex(point: point + translation, inTangent: inTangent + translation, outTangent: outTangent + translation)
  }
  ...
  /// S==NO=========TI==T==TO=======NI==E
  func splitCurve(toVertex: NECurveVertex, position: CGFloat) ->
    (start: NECurveVertex, trimPoint: NECurveVertex, end: NECurveVertex) {
    /// If position is less than or equal to 0, trim at start.
    if position <= 0 {
      return (
        start: NECurveVertex(point: point, inTangentRelative: inTangentRelative, outTangentRelative: .zero),
        trimPoint: NECurveVertex(point: point, inTangentRelative: .zero, outTangentRelative: outTangentRelative),
        end: toVertex
      )
    }
    ...
    if outTangentRelative.isZero, toVertex.inTangentRelative.isZero {
      ...
      return (
        start: self,
        trimPoint: NECurveVertex(point: trimPoint, inTangentRelative: .zero, outTangentRelative: .zero),
        end: toVertex
      )
    }
    ...
    return (
      start: NECurveVertex(point: point, inTangent: inTangent, outTangent: a),
      trimPoint: NECurveVertex(point: f, inTangent: d, outTangent: e),
      end: NECurveVertex(point: toVertex.point, inTangent: c, outTangent: toVertex.outTangent)
    )
  }
  ...
  /// length.
  ///
  /// This function should probably live in NEPathElement, since it deals with curve
  /// lengths.
  func trimCurve(toVertex: NECurveVertex, atLength: CGFloat, curveLength: CGFloat, maxSamples: Int, accuracy: CGFloat = 1) ->
    (start: NECurveVertex, trimPoint: NECurveVertex, end: NECurveVertex) {
  ...
  func distanceTo(_ toVertex: NECurveVertex, sampleCount: Int = 25) -> CGFloat {
    if outTangentRelative.isZero, toVertex.inTangentRelative.isZero {
