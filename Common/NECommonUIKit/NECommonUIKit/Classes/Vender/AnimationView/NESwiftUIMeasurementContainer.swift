...
// found in the LICENSE file.
  ...
  // MARK: - NESwiftUIMeasurementContainer
  ...
  ///
  /// - SeeAlso: ``NEMeasuringViewRepresentable``
  @available(iOS 13.0, tvOS 13.0, macOS 10.15, *)
  final class NESwiftUIMeasurementContainer<Content: NEViewType>: NEViewType {
    // MARK: Lifecycle

    init(content: Content, strategy: NESwiftUIMeasurementContainerStrategy) {
    ...
    ///
    /// Has a side-effect of updating the `measuredIntrinsicContentSize` if it's changed.
    var strategy: NESwiftUIMeasurementContainerStrategy {
      didSet {
    ...
    private var _resolvedStrategy: NEResolvedSwiftUIMeasurementContainerStrategy?
    ...
    private var resolvedStrategy: NEResolvedSwiftUIMeasurementContainerStrategy {
      if let resolvedStrategy = _resolvedStrategy {
        return resolvedStrategy
      }

      let resolved: NEResolvedSwiftUIMeasurementContainerStrategy
      switch strategy {
      case .automatic:
    ...
    private func prioritizeConstraints(_ constraints: [NSLayoutConstraint.Attribute: NSLayoutConstraint],
                                       strategy: NEResolvedSwiftUIMeasurementContainerStrategy) {
    ...
    private func measureView() -> CGSize {
      ...
      case .intrinsicHeightProposedWidth:
        ...
        measuredSize.width = NEViewType.noIntrinsicMetric

      case .intrinsicWidthProposedHeight:
        measuredSize = content.systemLayoutFittingIntrinsicWidthFixedHeight(proposedSizeElseBounds.height)
        measuredSize.height = NEViewType.noIntrinsicMetric
      ...
      case let .intrinsic(size):
        ...
        //   available.
        if
          proposedSize.width != NEViewType.noIntrinsicMetric,
          measuredSize.width > proposedSizeElseBounds.width,
          _intrinsicContentSize.width != proposedSize.width {
          measuredSize.width = NEViewType.noIntrinsicMetric
        }
        if
          proposedSize.height != NEViewType.noIntrinsicMetric,
  ...
  // MARK: - NESwiftUIMeasurementContainerStrategy

  /// The measurement strategy of a `NESwiftUIMeasurementContainer`.
  enum NESwiftUIMeasurementContainerStrategy {
  ...
  // MARK: - NEResolvedSwiftUIMeasurementContainerStrategy

  /// The resolved measurement strategy of a `NESwiftUIMeasurementContainer`, matching the cases of the
  /// `NESwiftUIMeasurementContainerStrategy` without the automatic case.
  private enum NEResolvedSwiftUIMeasurementContainerStrategy {
    case proposed, intrinsicHeightProposedWidth, intrinsicWidthProposedHeight, intrinsic(CGSize)
  }

  // MARK: - UILayoutPriority

  fileprivate extension NELayoutPriorityType {
    /// An "almost required" constraint, useful for creating near-required constraints that don't
    /// error when unable to be satisfied.
    @nonobjc
    static var almostRequired: NELayoutPriorityType { .init(rawValue: required.rawValue - 1) }
  }

  // MARK: - UIView

  fileprivate extension NEViewType {
    ...
    @nonobjc
    func systemLayoutFittingIntrinsicHeightFixedWidth(_ width: CGFloat,
                                                      priority: NELayoutPriorityType = .almostRequired)
      -> CGSize {
      #if os(macOS)
    ...
    @nonobjc
    func systemLayoutFittingIntrinsicWidthFixedHeight(_ height: CGFloat,
                                                      priority: NELayoutPriorityType = .almostRequired)
      -> CGSize {
      #if os(macOS)
  ...
  fileprivate extension CGSize {
    ...
    static var noIntrinsicMetric: CGSize {
      .init(width: NEViewType.noIntrinsicMetric, height: NEViewType.noIntrinsicMetric)
    }
    ...
    func replacingNoIntrinsicMetric(with fallback: CGSize) -> CGSize {
      .init(
        width: width == NEViewType.noIntrinsicMetric ? fallback.width : width,
        height: height == NEViewType.noIntrinsicMetric ? fallback.height : height
      )
    }
  }
#endif
