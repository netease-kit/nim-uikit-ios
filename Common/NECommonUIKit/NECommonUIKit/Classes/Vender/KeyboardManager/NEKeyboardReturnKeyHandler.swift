...
private final class NETextFieldViewInfoModal: NSObject {
 ...
 */
@available(iOSApplicationExtension, unavailable)
@objc public final class NEKeyboardReturnKeyHandler: NSObject {
  // MARK: Settings
  ...
  private var textFieldInfoCache = [NETextFieldViewInfoModal]()

  // MARK: Private Functions

  private func textFieldViewCachedInfo(_ textField: UIView) -> NETextFieldViewInfoModal? {
    for modal in textFieldInfoCache {
      if let view = modal.textFieldView {
        if view == textField {
          return modal
  ...
  private func updateReturnKeyTypeOnTextField(_ view: UIView) {
    ...
    // (Bug ID: #347)
    for disabledClass in NEKeyboardManager.shared.toolbarPreviousNextAllowedClasses {
      superConsideredView = view.neSuperviewOfClassType(disabledClass)
    ...
    if let unwrappedTableView = superConsideredView { //   (Enhancement ID: #22)
      textFields = unwrappedTableView.neDeepResponderViews()
    } else { // Otherwise fetching all the siblings
      textFields = view.neResponderSiblings()

      // Sorting textFields according to behaviour
      switch NEKeyboardManager.shared.toolbarManageBehaviour {
      // If needs to sort it by tag
      case .byTag: textFields = textFields.neSortedArrayByTag()
      // If needs to sort it by Position
      case .byPosition: textFields = textFields.neSortedArrayByPosition()
      default: break
      }
    }
    ...
    if let lastView = textFields.last {
      if let textField = view as? UITextField {
        ...
        textField
          .returnKeyType = (view == lastView) ? lastTextFieldReturnKeyType : UIReturnKeyType
          .next
  ...
  @objc public func addTextFieldView(_ view: UIView) {
    let modal = NETextFieldViewInfoModal(
      textFieldView: view,
  ...
  @objc public func addResponderFromView(_ view: UIView) {
    let textFields = view.neDeepResponderViews()
...
@available(iOSApplicationExtension, unavailable)
extension NEKeyboardReturnKeyHandler: UITextFieldDelegate {
  @objc public func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textField)?.textFieldDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextFieldDelegate.textFieldShouldBeginEditing(_:))) {
  ...
  @objc public func textFieldShouldEndEditing(_ textField: UITextField) -> Bool {
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textField)?.textFieldDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextFieldDelegate.textFieldShouldEndEditing(_:))) {
  ...
  @objc public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange,
                              replacementString string: String) -> Bool {
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textField)?.textFieldDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextFieldDelegate
              .textField(_:shouldChangeCharactersIn:replacementString:))) {
          return unwrapDelegate.textField?(
            textField,
  ...
  @objc public func textFieldShouldClear(_ textField: UITextField) -> Bool {
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textField)?.textFieldDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextFieldDelegate.textFieldShouldClear(_:))) {
  ...
  @objc public func textFieldShouldReturn(_ textField: UITextField) -> Bool {
    ...
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textField)?.textFieldDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextFieldDelegate.textFieldShouldReturn(_:))) {
...
@available(iOSApplicationExtension, unavailable)
extension NEKeyboardReturnKeyHandler: UITextViewDelegate {
  @objc public func textViewShouldBeginEditing(_ textView: UITextView) -> Bool {
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textView)?.textViewDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextViewDelegate.textViewShouldBeginEditing(_:))) {
  ...
  @objc public func textViewShouldEndEditing(_ textView: UITextView) -> Bool {
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textView)?.textViewDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextViewDelegate.textViewShouldEndEditing(_:))) {
  ...
  @objc public func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange,
    ...
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(textView)?.textViewDelegate {
        if unwrapDelegate
          .responds(to: #selector(UITextViewDelegate
              .textView(_:shouldChangeTextIn:replacementText:))) {
          shouldReturn = (unwrapDelegate
            .textView?(textView, shouldChangeTextIn: range, replacementText: text)) ??
            false
        }
      }
    }
  ...
  @available(iOS 10.0, *)
  @objc public func textView(_ aTextView: UITextView, shouldInteractWith URL: URL,
                             in characterRange: NSRange,
                             interaction: UITextItemInteraction) -> Bool {
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(aTextView)?.textViewDelegate {
        if unwrapDelegate
          .responds(to: #selector(textView as (UITextView, URL, NSRange,
                                               UITextItemInteraction) -> Bool)) {
          return unwrapDelegate.textView?(
            aTextView,
  ...
  @objc public func textView(_ aTextView: UITextView,
    ...
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(aTextView)?.textViewDelegate {
        if unwrapDelegate
          .responds(to: #selector(textView as (UITextView, NSTextAttachment, NSRange,
                                               UITextItemInteraction) -> Bool)) {
          return unwrapDelegate.textView?(
            aTextView,
  ...
  @objc public func textView(_ aTextView: UITextView, shouldInteractWith URL: URL,
    ...
    if delegate == nil {
      if let unwrapDelegate = textFieldViewCachedInfo(aTextView)?.textViewDelegate {
        if unwrapDelegate
          .responds(to: #selector(textView as (UITextView, URL, NSRange) -> Bool)) {
          return unwrapDelegate.textView?(
            aTextView,
            shouldInteractWith: URL,
            in: characterRange
