...
open class NEBaseTeamManagerListController: NETeamBaseViewController, UITableViewDelegate, UITableViewDataSource, TeamMemberCellDelegate, TeamManagerListViewModelDelegate {
  ...
  open func didAddManagers(_ managers: [NETeamMemberInfoModel]) {
    if NEChatDetectNetworkTool.shareInstance.manager?.isReachable == false {
      view.neMakeToast(commonLocalizable("network_error"))
      return
    }
    ...
    if let tid = teamId {
      ...
      viewmodel.addTeamManager(tid, uids) { [weak self] error in
        if error != nil {
          self?.view.neMakeToast(commonLocalizable("failed_operation"))
        } else {
    ...
    showAlert(title: localizable("remove_manager_title"), message: localizable("remove_manager_tip")) {
      ...
      if let tid = weakSelf?.teamId, let uid = model?.nimUser?.user?.accountId {
        weakSelf?.viewmodel.removeTeamManager(tid, [uid]) { error in
          if let err = error {
            weakSelf?.view.neMakeToast(err.localizedDescription)
          } else {
