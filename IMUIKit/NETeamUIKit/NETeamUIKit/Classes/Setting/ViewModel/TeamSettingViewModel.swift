...
@objcMembers
open class TeamSettingViewModel: NSObject, NETeamListener {
  /// 分区区域数据
  ...
  override public init() {
    ...
    teamInfo.team = NETeamUserManager.shared.getTeamInfo()
      ...
      if let teamId = weakSelf?.teamInfoModel?.team?.teamId, let conversationId = V2NIMConversationIdUtil.teamConversationId(teamId) {
        if NIMSDK.shared().v2Option?.enableV2CloudConversation == false {
          weakSelf?.localConversationRepo.setStickTop(conversationId, isOpen) { error in
            NEALog.infoLog(TeamSettingViewModel.className(), desc: #function + (isOpen ? "addStickTop" : "removeStickTop") + " error : \(error?.localizedDescription ?? "") ")
            if let err = error {
              weakSelf?.delegate?.didNeedRefreshUI()
              weakSelf?.delegate?.didError(err)
            } else {
  ...
  open func sampleMemberId(arr: [NETeamMemberInfoModel], owner: String) -> String? {
  ...
  /// 移交群主
  /// - Parameter completion: 完成回调
  open func transferTeamOwner(_ completion: @escaping (Error?) -> Void) {
    if isOwner() == false {
    ...
    for member in members {
      ...
      if member.accountId == IMKitClient.instance.account(), let teamId = teamInfoModel?.team?.teamId {
        getCurrentMember(IMKitClient.instance.account(), teamId) { [weak self] member, error in
          NEALog.infoLog(TeamSettingViewModel.className(), desc: "current member : \(self?.memberInTeam?.yx_modelToJSONString() ?? "")")
        }
      }
    }
