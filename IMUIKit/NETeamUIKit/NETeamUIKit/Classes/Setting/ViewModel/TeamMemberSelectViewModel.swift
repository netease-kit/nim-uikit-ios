...
@objc
public protocol TeamMemberSelectViewModelDelegate: NSObjectProtocol {
  func didNeedRefresh()
}
...
class TeamMemberSelectViewModel: NSObject, NETeamListener, NETeamChatUserCacheListener {
  ...
  open func onTeamMemberUpdate(_ accountId: String) {
    if let tid = teamInfoModel?.team?.teamId {
      ...
      weak var weakSelf = self
      getTeamInfo(tid, showAllMembers) { error in
        if error == nil {
          self.delegate?.didNeedRefresh()
        } else {
          NEALog.infoLog(TeamMemberSelectViewModel.className(), desc: #function + "memberCacheDidChange get team info error \(error?.localizedDescription ?? ""))")
        }
      }
    }
  }
  ...
  /// 所有展示数据
  /// - Parameter searchText: 搜索关键字
  open func searchShowData(_ searchText: String) -> [NESelectTeamMember] {
  ...
  /// 群成员变更统一处理
  /// - Parameter teamMembers: 群成员
  func onTeamMemberChanged(_ members: [V2NIMTeamMember]) {
    ...
    if isCurrentTeam == true {
      if let tid = teamInfoModel?.team?.teamId {
        getTeamInfo(tid, showAllMembers) { [weak self] error in
          if error == nil {
            self?.delegate?.didNeedRefresh()
          }
        }
  ...
  open func getTeamMembers(_ teamInfo: NETeamInfoModel,
    ...
    if let members = NETeamUserManager.shared.getAllTeamMemberModels() {
      ...
      NEALog.infoLog(TeamMemberSelectViewModel.className(), desc: "load team member from cache success.")
      return
    }
