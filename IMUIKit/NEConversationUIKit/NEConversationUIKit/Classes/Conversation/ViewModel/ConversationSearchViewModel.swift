...
open class ConversationSearchViewModel: NSObject, NETeamListener, NEIMKitClientListener {
  ...
  override public init() {
    ...
    getSearchData {
      NEALog.infoLog(ModuleName + " " + ConversationSearchViewModel.className(), desc: "get data finish")
    }
  }
  ...
  func getSearchData(_ removeTeamData: Bool = false, _ completion: @escaping () -> Void) {
    ...
    workingQueue.async {
      ...
      NEALog.infoLog(ConversationSearchViewModel.className(), desc: #function + "conversation search get friend list ")
      workingGroup.leave()
    }
    ...
    workingQueue.async {
      TeamRepo.shared.getTeamList { teams, error in
        NEALog.infoLog(ConversationSearchViewModel.className(), desc: #function + " conversation search get team list \(error?.localizedDescription ?? "")")
  ...
  /// 移除群
  /// - Parameter team: 群
  public func removeTeam(_ team: V2NIMTeam) {
    teamDic.removeValue(forKey: team.teamId)
  }

  /// 添加群
  /// - Parameter team: 群
  public func addTeam(_ team: V2NIMTeam) {
    let model = ConversationSearchListModel()
    model.team = team
  ...
  open func onTeamSyncFinished() {
    ...
    getSearchData(true) {
      NEALog.infoLog(ConversationSearchViewModel.className(), desc: #function + ", get data finish")
    }
  }
}
