...
open class TeamChatViewModel: ChatViewModel, NETeamListener {
  ...
  open func markReadInTeam(messages: [V2NIMMessage], _ completion: @escaping (Error?) -> Void) {
    if !ChatUIConfig.shared.messageProperties.showTeamMessageStatus {
      completion(nil)
      return
    }
  ...
  open func getTeamMessageReceipts(messages: [V2NIMMessage], _ completion: @escaping ([IndexPath], Error?) -> Void) {
    ...
    let group = DispatchGroup()
    let chunkMessages = sendMessages.chunk(50)
    var reloadIndexs = [IndexPath]()
    var err: NSError?
    var readReceiptList: [V2NIMTeamMessageReadReceipt] = []
    for chunkMessage in chunkMessages {
      group.enter()
      chatRepo.getTeamMessageReceipts(messages: chunkMessage) { readReceipts, error in
        if let e = error {
          err = e
        }

        if let readReceipts = readReceipts {
          readReceiptList.append(contentsOf: readReceipts)
        }
    ...
    group.notify(queue: .main, work: DispatchWorkItem(block: {
      for readReceipt in readReceiptList {
