...
open class ChatViewController: NEChatBaseViewController, UINavigationControllerDelegate, UITableViewDataSource, UITableViewDelegate, UIDocumentPickerDelegate, UIDocumentInteractionControllerDelegate, NIMMediaManagerDelegate, CLLocationManagerDelegate, UIImagePickerControllerDelegate, ChatInputViewDelegate, ChatInputMultilineDelegate, ChatViewModelDelegate, MessageOperationViewDelegate, NETranslateViewDelegate, SelectLanguageDelegate {
  ...
  public lazy var chatInputView: NEBaseChatInputView = {
    let conversationType = V2NIMConversationIdUtil.conversationType(ChatRepo.conversationId)
  ...
  public var jumpDownViewRightAncher: NSLayoutConstraint?
  public lazy var jumpDownView: NEBaseChatNewMessageView = {
    let view = getJumpDownView()
    view.translatesAutoresizingMaskIntoConstraints = false
    ...
    let tap = UITapGestureRecognizer(target: self, action: #selector(jumpDownMessage))
      ...
      if weakSelf?.viewModel.messages.isEmpty == false {
        ...
        if weakSelf?.viewModel.isHistoryChat == true,
          ...
          if newEnd > 0 {
            ...
            // 无更多新消息，则按照首次拉取处理
            weakSelf?.viewModel.isHistoryChat = false
          }
          // showJumpDownView 移到 loadDataFinish 中统一处理，避免在合并 pendingNewMessages 之前就清空 onReceiveNewMsgs
        } else {
          // 首次拉取
  ...
  open func markNeedReadMsg(_ messages: [V2NIMMessage]) {
    if isCurrentPage, !messages.isEmpty {
      viewModel.markRead(messages: messages) { [weak self] error in
        NEALog.infoLog(
          ModuleName + " " + ChatViewController.className(),
  ...
  open func didLongTouchMessageView(_ cell: UITableViewCell, _ model: MessageContentModel?) {
    ...
    if viewModel.operationModel != model {
      (viewModel.operationModel?.cell as? NEBaseChatMessageCell)?.resetSelectRange()
  ...
  override open func toSetting() {
    ...
    if V2NIMConversationIdUtil.conversationType(ChatRepo.conversationId) == .CONVERSATION_TYPE_TEAM {
      Router.shared.use(
    ...
    } else if V2NIMConversationIdUtil.conversationType(ChatRepo.conversationId) == .CONVERSATION_TYPE_P2P {
      let userSetting = getUserSettingViewController()
      navigationController?.pushViewController(userSetting, animated: true)
    }
  }

  open func jumpDownMessage() {
    if viewModel.isHistoryChat {
      // 历史聊天模式下，重置锚点并重新加载最新消息
      // 不在此处清空 onReceiveNewMsgs，等待 loadData 异步完成后由 loadDataFinish 统一处理
    ...
    showJumpDownMessage(-1)
    markNeedReadMsg(onReceiveNewMsgs)
    onReceiveNewMsgs.removeAll()
  }

  open func showJumpDownMessage(_ messageCount: Int) {
    if messageCount > 0 {
      jumpDownView.updateMessageCount(messageCount)
      jumpDownViewRightAncher?.constant = 16
    } else if messageCount == 0 {
      jumpDownView.updateMessageCount(messageCount)
      jumpDownViewRightAncher?.constant = -16
    } else {
      jumpDownView.updateMessageCount(-1)
    }
  }
  ...
  open func keyBoardWillShow(_ notification: Notification) {
    ...
    // 任何时候弹起键盘都应该滚动到底部
    // 如果是历史聊天模式，scrollTableViewToBottom 会先调用 dataReload() 加载最新消息
    layoutInputViewWithAnimation(offset: keyboardRect.size.height, animationDuration, true)
  }
  ...
  open func sendContentText(text: String?, attribute: NSAttributedString?) {
    guard let removeSpace = text?.trimmingCharacters(in: .whitespaces), !removeSpace.isEmpty else {
      ...
      view.neMakeToast(chatLocalizable("null_message_not_support"), position: .center)
      return
    }
...
//                                        }))
  ...
  open func sending(_ message: V2NIMMessage, _ index: IndexPath) {
    let scrollToBottom = isCurrentPage && presentedViewController == nil
    insertRows([index], scrollToBottom)

    if !scrollToBottom, !isMutilSelect {
      showJumpDownMessage(onReceiveNewMsgs.count)
    }
  }
  ...
  open func onRevokeMessage(_ message: V2NIMMessage, atIndexs: [IndexPath]) {
    ...
    tableViewReloadIndexs(atIndexs) { [weak self] in
      for index in atIndexs {
        if let numberOfRows = self?.tableView.numberOfRows(inSection: 0),
           index.row == numberOfRows - 1,
           let cell = self?.tableView.cellForRow(at: index),
           self?.tableView.visibleCells.contains(cell) == true {
          self?.scrollTableViewToBottom()
        }
      }
    }
    ...
    onReceiveNewMsgs.removeAll { $0.messageClientId == message.messageClientId }
    showJumpDownView()
  }
  ...
  open func tableViewReload() {
    ...
    // 检查是否需要在数据加载完成后显示跳转按钮
    if needCheckJumpDownAfterReload {
      needCheckJumpDownAfterReload = false
  ...
  open func startPlay(cell: ChatAudioCellProtocol?, model: MessageAudioModel?) {
    let isSend = ChatMessageHelper.isSelf(message: model?.message)
  ...
  open func stopPlay() {
    audioPlayer?.stop()
    audioPlayer = nil
    playingCell?.stopAnimation(byRight: ChatMessageHelper.isSelf(message: playingModel?.message))
    playingModel = nil
    ...
    viewModel.topMessage { [weak self] error in
      ...
      if let err = error as? NSError {
        if err.code == protocolSendFailed {
          ...
          self?.view.neMakeToast(chatLocalizable("no_permission_tip"), position: .center)
        } else {
          self?.view.neMakeToast(error?.localizedDescription, position: .center)
        }
      }
    ...
    viewModel.untopMessage { [weak self] error in
      ...
      if let err = error as? NSError {
        if err.code == protocolSendFailed {
          ...
          self?.view.neMakeToast(commonLocalizable("failed_operation"), position: .center)
        } else {
          self?.view.neMakeToast(error?.localizedDescription, position: .center)
        }
      }
  ...
  open func cancelMutilSelect() {
    ...
    viewModel.messages.forEach { $0.isSelected = false }
    setMoreButton()
  ...
  open func tableView(_ tableView: UITableView,
    ...
    var isSend = ChatMessageHelper.isSelf(message: model.message)
...
//    }
  ...
  open func getMenuView(_ conversationType: V2NIMConversationType) -> NEBaseChatInputView {
  ...
  open func expandMoreAction() -> [NEMoreItemModel] {
    var items = NEChatUIKitClient.instance.getMoreActionData(sessionType: V2NIMConversationIdUtil.conversationType(ChatRepo.conversationId), self)
  ...
  open func didTapImageMessage(_ model: MessageContentModel?, _ replyIndex: Int? = nil) {
    ...
    if !imageUrl.isEmpty {
      var (currenIndex, showImages) = ChatMessageHelper.getUrls(model, viewModel.messages)
      ...
      let showController = PhotoBrowserController(urls: showImages, url: imageUrl, currenIndex)
  ...
  open func downloadFile(_ cell: UITableViewCell?, _ model: MessageContentModel?, _ url: String?, _ path: String) {
    ...
    var isSend = ChatMessageHelper.isSelf(message: fileModel.message)
  ...
  open func didClickMultiForwardButton() {
    ...
    for msg in viewModel.selectedMessages {
      if msg.sendingState == .MESSAGE_SENDING_STATE_FAILED ||
        msg.messageStatus.errorCode != operationSuccess
//            || msg.isBlackListed
      {
...
extension ChatViewController: ChatBaseCellDelegate {
  open func didLongPressAvatar(_ cell: UITableViewCell, _ model: MessageContentModel?) {
    ...
    // 禁言
    // 多选
    guard V2NIMConversationIdUtil.conversationType(ChatRepo.conversationId) == .CONVERSATION_TYPE_TEAM,
          !isMute,
          !isMutilSelect else {
      return
    }
  ...
  open func didTapMessageView(_ cell: UITableViewCell, _ model: MessageContentModel?, _ replyModel: MessageModel?) {
    ...
    guard let model = model,
          let cell = cell as? NEBaseChatMessageCell,
          model.isRevoked == false else {
      return
    }
  ...
  open func messageWillShow(_ cell: UITableViewCell, _ model: MessageContentModel?) {
    if let message = model?.message {
      ...
      if isCurrentPage,
        ...
        // 移除 onReceiveNewMsgs 的逻辑已移至 tableView(_:willDisplay:forRowAt:) 中
        // 因为 messageWillShow 在 cellForRowAt 的 setModel 中被调用（cell 预加载时）
        // 而 willDisplay 是 cell 真正即将显示在屏幕上时才调用
      }
...
extension ChatViewController: NEIMKitClientListener {
  ...
  open func onConnectStatus(_ status: V2NIMConnectStatus) {
    ...
    if status == .CONNECT_STATUS_CONNECTED, networkBroken {
      networkBroken = false
      if isCurrentPage {
        markNeedReadMsg(needMarkReadMsgs)
        needMarkReadMsgs.removeAll()
      }
