...
@objcMembers
open class NEBasePinMessageViewController: NEChatBaseViewController, UITableViewDataSource, UITableViewDelegate, PinMessageViewModelDelegate, PinMessageCellDelegate, UIDocumentInteractionControllerDelegate, NEIMKitClientListener, NEChatListener {
  ...
  func setupUI() {
    ...
    emptyView.setText(chatLocalizable("no_pin_message"))
  ...
  open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    // 合并 ChatVC 和 PinMessageVC 的新消息缓存
    // 因为两者可能同时在监听消息，需要合并去重
    // 重要：以 ChatVC 的缓存为基础，然后合并 PinVC 的新消息
    // 这样可以保留进入标记列表之前 ChatVC 已有的新消息计数
    ...
    // 然后合并 PinVC 中的新消息（按消息ID去重）
    let existingIds = Set(newMsgs.compactMap(\.messageServerId))
    for msg in onReceiveNewMsgs {
      if let msgId = msg.messageServerId, !existingIds.contains(msgId) {
        newMsgs.append(msg)
      }
    }
    ...
    if item.message.conversationType == .CONVERSATION_TYPE_P2P {
      ...
      // 只清空 PinVC 的缓存，避免再次进入标记列表时重复计数
      // 不清空 ChatVC 的缓存，因为 Router 在复用 ChatVC 时不会设置 pendingNewMessages
      // ChatVC 的 onReceiveNewMsgs 保持不变
  ...
  open func downloadFile(_ fileModel: PinMessageFileModel, _ urlString: String, _ path: String) {
    ...
    viewModel.downLoad(urlString, path) { progress in
      NEALog.infoLog(ModuleName + " " + NEBasePinMessageViewController.className(), desc: #function + "downLoad file progress: \(progress)")
  ...
  /// - Parameter model: 标记对象
  public func didPlay(cell: NEBasePinMessageCell?, model: NEPinMessageModel?) {
  ...
  /// - Parameter cell: 标记列表视图对象
  /// - Parameter model: 标记对象
  public func startPlay(cell: NEBasePinMessageCell?, model: NEPinMessageModel?) {
