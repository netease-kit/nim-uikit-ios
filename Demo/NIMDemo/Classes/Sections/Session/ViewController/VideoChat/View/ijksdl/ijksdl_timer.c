...
{
    profiler->begin_time = NTES_SDL_GetTickHR();
}

int64_t SDL_ProfilerEnd(SDL_Profiler* profiler)
{
    int64_t delta = NTES_SDL_GetTickHR() - profiler->begin_time;
...
{
    Uint64 current = NTES_SDL_GetTickHR();
...
void SDL_SpeedSampler2Reset(SDL_SpeedSampler2 *sampler, int sample_range)
{
    ...
    sampler->last_profile_tick = (int64_t)NTES_SDL_GetTickHR();
}
...
int64_t SDL_SpeedSampler2Add(SDL_SpeedSampler2 *sampler, int quantity)
{
    ...
    int64_t now           = (int64_t)NTES_SDL_GetTickHR();
