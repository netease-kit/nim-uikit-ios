...
- (void)dealloc
{
    ...
    NTES_IJK_GLES2_Renderer_reset(_renderer);
    NTES_IJK_GLES2_Renderer_freeP(&_renderer);
...
- (BOOL)setupRenderer: (SDL_VoutOverlay *) overlay
{
    ...
    if (!NTES_IJK_GLES2_Renderer_isValid(_renderer) ||
        !NTES_IJK_GLES2_Renderer_isFormat(_renderer, overlay->format)) {

        NTES_IJK_GLES2_Renderer_reset(_renderer);
        NTES_IJK_GLES2_Renderer_freeP(&_renderer);

        _renderer = NTES_IJK_GLES2_Renderer_create(overlay);
        if (!NTES_IJK_GLES2_Renderer_isValid(_renderer))
            return NO;

        if (!NTES_IJK_GLES2_Renderer_use(_renderer))
            return NO;

         NTES_IJK_GLES2_Renderer_setGravity(_renderer, _rendererGravity, _backingWidth, _backingHeight);
    }
...
- (void)displayInternal: (SDL_VoutOverlay *) overlay clear:(BOOL)clear
{
    ...
    if (!NTES_IJK_GLES2_Renderer_renderOverlay(_renderer, overlay))
    ...
    int64_t current = (int64_t)NTES_SDL_GetTickHR();
