...
IJK_GLES2_Renderer *NTES_IJK_GLES2_Renderer_create_rgb565()
{
    ...
    IJK_GLES2_Renderer *renderer = NTES_IJK_GLES2_Renderer_create_base(NTES_IJK_GLES2_getFragmentShader_rgb());
    if (!renderer)
        goto fail;
...
static GLsizei bgra8888_getBufferWidth(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!overlay)
        return 0;
...
static GLboolean bgra8888_uploadTexture(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!renderer || !overlay)
        return GL_FALSE;
    ...
    switch (overlay->format) {
        case SDL_FCC_BGRA:
            break;
        default:
            ALOGE("[bgra8888] unexpected format %x\n", overlay->format);
            return GL_FALSE;
    }
...
IJK_GLES2_Renderer *NTES_IJK_GLES2_Renderer_create_bgra8888()
{
    ...
    renderer->func_use            = rgb_use;
    renderer->func_getBufferWidth = bgra8888_getBufferWidth;
    renderer->func_uploadTexture  = bgra8888_uploadTexture;
