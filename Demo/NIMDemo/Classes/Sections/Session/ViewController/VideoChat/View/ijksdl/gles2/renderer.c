...
IJK_GLES2_Renderer *NTES_IJK_GLES2_Renderer_create_base(const char *fragment_shader_source)
{
    ...
    renderer->vertex_shader = NTES_IJK_GLES2_loadShader(GL_VERTEX_SHADER, NTES_IJK_GLES2_getVertexShader_default());
    if (!renderer->vertex_shader)
        goto fail;
    
    renderer->fragment_shader = NTES_IJK_GLES2_loadShader(GL_FRAGMENT_SHADER, fragment_shader_source);
    if (!renderer->fragment_shader)
        goto fail;
    
    renderer->program = glCreateProgram();                          NTES_IJK_GLES2_checkError("glCreateProgram");
    if (!renderer->program)
        goto fail;
    
    glAttachShader(renderer->program, renderer->vertex_shader);     NTES_IJK_GLES2_checkError("glAttachShader(vertex)");
    glAttachShader(renderer->program, renderer->fragment_shader);   NTES_IJK_GLES2_checkError("glAttachShader(fragment)");
    glLinkProgram(renderer->program);                               NTES_IJK_GLES2_checkError("glLinkProgram");
...
IJK_GLES2_Renderer *NTES_IJK_GLES2_Renderer_create(SDL_VoutOverlay *overlay)
{
    ...
    NTES_IJK_GLES2_printString("Version", GL_VERSION);
    NTES_IJK_GLES2_printString("Vendor", GL_VENDOR);
    NTES_IJK_GLES2_printString("Renderer", GL_RENDERER);
    NTES_IJK_GLES2_printString("Extensions", GL_EXTENSIONS);
    
    IJK_GLES2_Renderer *renderer = NULL;
    switch (overlay->format){
        case SDL_FCC_RV16:      renderer = NTES_IJK_GLES2_Renderer_create_rgb565(); break;
        case SDL_FCC_RV24:      renderer = NTES_IJK_GLES2_Renderer_create_rgb888(); break;
        case SDL_FCC_RV32:      renderer = NTES_IJK_GLES2_Renderer_create_rgbx8888(); break;
        case SDL_FCC_BGRA:      renderer = NTES_IJK_GLES2_Renderer_create_bgra8888(); break;
#ifdef __APPLE__
        case SDL_FCC_NV12:      renderer = NTES_IJK_GLES2_Renderer_create_yuv420sp(); break;
#ifdef NTESIJKSDLHasVTB
        case SDL_FCC__VTB:      renderer = NTES_IJK_GLES2_Renderer_create_yuv420sp_vtb(overlay); break;
#endif
#endif
        case SDL_FCC_YV12:      renderer = NTES_IJK_GLES2_Renderer_create_yuv420p(); break;
        case SDL_FCC_I420:      renderer = NTES_IJK_GLES2_Renderer_create_yuv420p(); break;
        case SDL_FCC_I444P10LE: renderer = NTES_IJK_GLES2_Renderer_create_yuv444p10le(); break;
        default:
...
{
    if (!NTES_IJK_GLES2_Renderer_isValid(renderer))
        return GL_FALSE;
...
GLboolean  NTES_IJK_GLES2_Renderer_setGravity(IJK_GLES2_Renderer *renderer, int gravity, GLsizei layer_width, GLsizei layer_height)
{
...
GLboolean NTES_IJK_GLES2_Renderer_use(IJK_GLES2_Renderer *renderer)
{
    ...
    NTES_IJK_GLES2_loadOrtho(&modelViewProj, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f);
