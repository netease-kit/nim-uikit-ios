...
static GLboolean yuv444p10le_use(IJK_GLES2_Renderer *renderer)
{
    ...
    glUniformMatrix3fv(renderer->um3_color_conversion, 1, GL_FALSE, NTES_IJK_GLES2_getColorMatrix_bt709());
...
IJK_GLES2_Renderer *NTES_IJK_GLES2_Renderer_create_yuv444p10le()
{
    ...
    IJK_GLES2_Renderer *renderer = NTES_IJK_GLES2_Renderer_create_base(NTES_IJK_GLES2_getFragmentShader_yuv444p10le());
    if (!renderer)
        goto fail;
