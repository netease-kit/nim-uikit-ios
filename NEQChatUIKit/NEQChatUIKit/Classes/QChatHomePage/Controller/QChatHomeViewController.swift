...
//    MARK: action
  ...
  @objc func onCreateServer(noti: Notification) {
    ...
    viewModel.createChannel { error, channel in
      if error == nil {
        NELog.infoLog(ModuleName + " " + className, desc: "✅CALLBACK second_channel create success")
        ...
        viewModel.createChannel { error, channel in
          if let err = error {
            ...
            NELog.infoLog(ModuleName + " " + className, desc: "✅CALLBACK enter first channel success")
