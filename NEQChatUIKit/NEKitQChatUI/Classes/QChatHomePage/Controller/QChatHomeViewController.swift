...
public class QChatHomeViewController: UIViewController, ViewModelDelegate {
    ...
    private lazy var brokenNetworkView:NEBrokenNetworkView = {
        let view = NEBrokenNetworkView.init(frame: CGRect.init(x: 0, y: 38, width: qChatBgView.width, height: 33))
        return view
    }()
}
    ...
    @objc func onCreateServer(noti: Notification) {
        ...
        viewModel.createChannel { error, channel in
            if error == nil {
                ...
                viewModel.createChannel { error, channel in
                    if let err = error {
                        NELog.errorLog(className, desc: "❌createChannel first_channel failed，error = \(err)")
                    }else {
