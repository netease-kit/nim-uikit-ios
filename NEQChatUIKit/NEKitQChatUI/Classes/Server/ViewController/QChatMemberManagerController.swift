...
public class QChatMemberManagerController: NEBaseTableViewController {
    ...
    func setupUI(){
        self.title = localizable("qchat_manager_member")
        view.backgroundColor = .white
        setupTable()
        tableView.delegate = self
        tableView.dataSource = self
        tableView.register(QChatMemberManagerCell.self, forCellReuseIdentifier: "\(QChatMemberManagerCell.self)")
        tableView.register(QChatPlainTextArrowCell.self, forCellReuseIdentifier: "\(QChatPlainTextArrowCell.self)")
...
extension QChatMemberManagerController: UITableViewDelegate, UITableViewDataSource, ViewModelDelegate, QChatMemberSelectControllerDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 1 {
            let cell: QChatMemberManagerCell = tableView.dequeueReusableCell(withIdentifier: "\(QChatMemberManagerCell.self)", for: indexPath) as! QChatMemberManagerCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            memberSelect.completion = { users in
                ...
                weakSelf?.viewmodel.addMembers(users, weakSelf?.serverId, weakSelf?.roleId) { successCount in
            ...
            showAlert(message: localizable("qchat_sure_delete_user")) {
                if let rid = weakSelf?.roleId, let sid = weakSelf?.serverId {
                    ...
                    weakSelf?.viewmodel.remove(user, sid, rid, {
                        ...
                        if var count = weakSelf?.memberCount, let block = weakSelf?.countChangeBlock {
                            count = count - 1
                            weakSelf?.memberCount = count
                            block(count)
                        }
