...
class PermissionModel: NSObject {
    ...
    let commonPermission = [#keyPath(PermissionModel.managerServer),#keyPath(PermissionModel.allChannelProperty), #keyPath(PermissionModel.role)]
    
    let commonPermissionDic = [#keyPath(PermissionModel.managerServer):localizable("qchat_manager_server"),#keyPath(PermissionModel.allChannelProperty):localizable("qchat_manager_channel"),
...
//    @objc var atAllMember = ChatPermissionType.remindAll.rawValue
    ...
    */
    ...
    let memberPermissionDic = [#keyPath(PermissionModel.modifyOwnServer):localizable("qchat_modify_own_server"),
    ...
    func getChangePermission() -> [ChatPermissionType: Bool] {
        ...
        changeMap.forEach { (key: String, v: Bool) in
            if let permissionKey = value(forKey: key)  as? String, let type = ChatPermissionType(rawValue: permissionKey) {
                permissions[type] = v
            }
        }
        return permissions
