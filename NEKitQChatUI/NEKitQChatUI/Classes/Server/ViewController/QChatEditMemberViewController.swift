...
//        }
    ...
    func setupUI(){
        ...
        tableView.register(QChatIdGroupSelectCell.self, forCellReuseIdentifier: "\(QChatIdGroupSelectCell.self)")
...
extension QChatEditMemberViewController: UITableViewDataSource, UITableViewDelegate, ViewModelDelegate, QChatTextEditCellDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell: QChatIdGroupSelectCell = tableView.dequeueReusableCell(withIdentifier: "\(QChatIdGroupSelectCell.self)", for: indexPath) as! QChatIdGroupSelectCell
            let group = showAll ? viewModel.allIdGroups[indexPath.row] : viewModel.limitIdGroups[indexPath.row]
            ...
            cell.contentLabel.text = showAll ? "收起(共\(viewModel.allIdGroups.count)个)" : "更多(共\(viewModel.allIdGroups.count)个)"
            if showAll {
            ...
            let cell: QChatDestructiveCell = tableView.dequeueReusableCell(withIdentifier: "\(QChatDestructiveCell.self)", for: indexPath) as! QChatDestructiveCell
            if indexPath.row == 0 {
                cell.cornerType = CornerType.topLeft.union(CornerType.topRight).union(CornerType.bottomLeft).union(CornerType.bottomRight)
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 1 {
            ...
            let group = showAll == true ? viewModel.allIdGroups[indexPath.row] :  viewModel.limitIdGroups[indexPath.row]
...
extension QChatEditMemberViewController  {
    ...
    @objc func rightBtnClick(_ btn: ExpandButton){
        if btn.isSelected == true {
            ...
            if IMKitLoginManager.instance.isMySelf(accid) == true {
                ...
                viewModel.updateMember(user?.serverMember?.serverId, nickName , user?.accid) { error, member in
                    if let err = error {
                        weakSelf?.showToast(err.localizedDescription)
                    }else {
                        weakSelf?.navigationController?.popViewController(animated: true)
