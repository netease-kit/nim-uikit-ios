...
public class CreateServerViewModel: QChatRepoMessageDelegate, AllChannelDataDelegate {
    ...
    public func onUnReadChange(_ unreads: [NIMQChatUnreadInfo]?, _ lastUnreads: [NIMQChatUnreadInfo]?) {
    ...
    public lazy var dataArray:[(String,String)] = {
        let array = [("mine_create",localizable("qchat_mine_add")),("addOther_icon",localizable("qchat_join_otherServer"))]
        return array
    }()
    

    public func createServer(parameter:CreateServerParam,_ completion:@escaping (NSError?,CreateServerResult?) ->()) {
    ...
    public func getServers(parameter:QChatGetServersParam,_ completion:@escaping(NSError?,QChatGetServersResult?)->() ){
        repo.getServers(parameter) { error, serverResult in
            completion(error,serverResult)
        }
    }
    
    public func getServerList(parameter:GetServersByPageParam,_ completion:@escaping (NSError?,GetServersByPageResult?)->()){
        QChatServerProvider.shared.getServerCount(param: parameter) { error, result in
            completion(error,result)
        }
    }
    
    public func getServerMemberList(parameter:QChatGetServerMembersParam,_ completion:@escaping (NSError?,QChatGetServerMembersResult?)->()) {
    ...
    public func getServerMembersByPage(parameter:QChatGetServerMembersByPageParam,_ completion:@escaping (NSError?,QChatGetServerMembersResult?)->()) {
...
//        repo.getServerMembersByPage(parameter, completion)
    ...
    public func applyServerJoin(parameter:QChatApplyServerJoinParam,_ completion:@escaping (NSError?)->()) {
    ...
    public func inviteMembersToServer(serverId:UInt64,accids:[String],_ completion:@escaping (NSError?)->()) {
