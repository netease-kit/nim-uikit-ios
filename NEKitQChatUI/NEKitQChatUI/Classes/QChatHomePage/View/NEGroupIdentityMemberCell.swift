...
class NEGroupIdentityMemberCell: UITableViewCell {
    ...
    public var memberModel:ServerMemeber?{
        didSet {
            ...
            if let imageName = model.avatar,!imageName.isEmpty {
                avatarImage.sd_setImage(with: URL.init(string: imageName), completed: nil)
    ...
    func setupSubviews(){
        ...
        NSLayoutConstraint.activate([
            avatarImage.topAnchor.constraint(equalTo: self.contentView.topAnchor,constant: 12),
        ...
        titleTopConstraint = titleLable.topAnchor.constraint(equalTo: self.contentView.topAnchor, constant: 14)
        ...
        NSLayoutConstraint.activate([
            ...
            arrowImageView.rightAnchor.constraint(equalTo:self.contentView.rightAnchor,constant: -labelHeight)
        ])

        
        NSLayoutConstraint.activate([
            labelContainerView.topAnchor.constraint(equalTo: avatarImage.bottomAnchor, constant: 8),
            labelContainerView.leftAnchor.constraint(equalTo: self.contentView.leftAnchor),
            labelContainerView.rightAnchor.constraint(equalTo: self.contentView.rightAnchor),
            labelContainerView.bottomAnchor.constraint(equalTo: self.contentView.bottomAnchor, constant: -10)
        ])
        
        //移除contentview上复用的lable
        self.labelContainerView.subviews.forEach { label in
        ...
        for i in (0..<dataArray.count) {
            ...
            if (maxWidth-labelsWidth) >= labelSize.width && isFirstRow{
                NSLayoutConstraint.activate([
                    i == 0 ? label.leftAnchor.constraint(equalTo: labelContainerView.leftAnchor,constant: kScreenInterval) : label.leftAnchor.constraint(equalTo: labelContainerView.leftAnchor,constant: kScreenInterval+labelsWidth),
                ...
                isFirstRow = false
                NSLayoutConstraint.activate([
                    label.leftAnchor.constraint(equalTo: labelContainerView.leftAnchor,constant: labelsWidth),
                    label.topAnchor.constraint(equalTo: labelContainerView.topAnchor,constant: 8+labelHeight+labelMargin),
                    label.widthAnchor.constraint(equalToConstant: labelSize.width),
                    label.heightAnchor.constraint(equalToConstant:labelSize.height)
                ])
            }
        ...
        NSLayoutConstraint.activate([
            ...
            lineView.rightAnchor.constraint(equalTo: self.contentView.rightAnchor),
            lineView.heightAnchor.constraint(equalToConstant: 1),
            lineView.leftAnchor.constraint(equalTo:self.contentView.leftAnchor,constant:kScreenInterval)
        ])
    }
    ...
    private lazy var arrowImageView:UIImageView = {
        let arrow = UIImageView.init(image: UIImage.ne_imageNamed(name: "arrowRight"))
...
class IDGroupLable:UILabel {
    ...
    override func draw(_ rect: CGRect) {
        super.draw(rect)
        self.addCorner(conrners: .allCorners, radius: 4)
    }
    
    func setupStyle(){
        self.font = DefaultTextFont(12)
        self.textColor = HexRGB(0x656A72)
        self.backgroundColor = HexRGB(0xF2F4F5)
        self.text = content
    }
