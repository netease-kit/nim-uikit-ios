...
class NEHomeChannelView: UIView {
    ...
    typealias SelectedChannelBlock = (_ channel: ChatChannel?) -> ()
    ...
    override func draw(_ rect: CGRect) {
        self.addCorner(conrners: [.topLeft,.topRight], radius: 8)
    }
    ...
    func setupSubviews(){
        ...
        NSLayoutConstraint.activate([
            titleLable.topAnchor.constraint(equalTo: self.topAnchor, constant: 16),
            titleLable.leftAnchor.constraint(equalTo: self.leftAnchor, constant: 12),
            titleLable.rightAnchor.constraint(equalTo: self.rightAnchor, constant: -30),

        ])

        NSLayoutConstraint.activate([
            setUpBtn.centerYAnchor.constraint(equalTo: titleLable.centerYAnchor),
            setUpBtn.rightAnchor.constraint(equalTo: self.rightAnchor, constant: -16),
        ])
        
        NSLayoutConstraint.activate([
            divideLineView.topAnchor.constraint(equalTo: titleLable.bottomAnchor,constant: 16),
            divideLineView.leftAnchor.constraint(equalTo: self.leftAnchor,constant: 12),
            divideLineView.rightAnchor.constraint(equalTo: self.rightAnchor, constant: -12),
        ...
        NSLayoutConstraint.activate([
            ...
            subTitleLable.leftAnchor.constraint(equalTo: self.leftAnchor, constant: 18),
        ])
        
        NSLayoutConstraint.activate([
            addChannelBtn.centerYAnchor.constraint(equalTo: subTitleLable.centerYAnchor),
            addChannelBtn.rightAnchor.constraint(equalTo: self.rightAnchor, constant: -15),
        ])
    ...
    public func channelChange(noticeInfo:NIMQChatSystemNotification) {
        ...
        case .channelRemove,.channelCreate,.channelUpdate:
            if noticeInfo.serverId == self.qchatServerModel?.serverId {
                requestData(timeTag: 0)
            }
            break
    ...
    lazy var tableView:UITableView = {
        ...
        tableView.register(NEHomeChannelCell.self, forCellReuseIdentifier: "\(NSStringFromClass(NEHomeChannelCell.self))")
    ...
    private lazy var emptyView:EmptyDataView = {
        let view = EmptyDataView(imageName: "channel_noMoreData", content: "该服务器下暂无频道",frame: tableView.bounds)
        view.translatesAutoresizingMaskIntoConstraints = false
        view.isHidden = true
        return view
    }()
...
extension NEHomeChannelView {
    ...
    @objc func loadMoreData(){
        self.requestData(timeTag: self.nextTimeTag)
    ...
    public func requestData(timeTag:TimeInterval){
        ...
        channelViewModel.getChannelsByPage(parameter: param) { [self] error, result in
            if error == nil {
                ...
                QChatLog.errorLog(self.className, desc: "❌getChannelsByPage failed,error = \(error!)")
            }
        }
    }
...
extension NEHomeChannelView:UITableViewDataSource,UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "\(NSStringFromClass(NEHomeChannelCell.self))", for: indexPath) as! NEHomeChannelCell
        if indexPath.row < channelArray.count {
            let channel = channelArray[indexPath.row]
            cell.channelModel = channel
            if let sid = qchatServerModel?.serverId, let cid = channel.channelId, let unreadCount = viewmodel?.getChannelUnreadCount(sid, cid) {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let block = self.selectedChannelBlock,channelArray.count>0{
