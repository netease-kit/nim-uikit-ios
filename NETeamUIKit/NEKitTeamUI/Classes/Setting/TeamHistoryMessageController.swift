...
public class TeamHistoryMessageController: NEBaseViewController {
    ...
    private lazy var searchTextField:SearchTextField = {
        ...
        let leftImageView = UIImageView.init(image: coreLoader.loadImage("search_icon"))
    ...
    public func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        weakSelf?.viewmodel.searchMessages(session, option: option, { error, messages in
            if error == nil {
                ...
                NELog.errorLog(weakSelf?.tag ?? "TeamHistoryMessageController", desc: "❌searchMessages failed, error = \(error!)")
            }
        })
