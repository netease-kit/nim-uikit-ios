...
open class TeamDetailHeaderView: UIView {
  ...
  open func getOwnerName(_ team: V2NIMTeam?, _ completion: @escaping (String) -> Void) {
    if let teamId = team?.teamId,
      ...
      TeamRepo.shared.getTeamMember(teamId, .TEAM_TYPE_NORMAL, ownerAccountId) { member, error in
        if let err = error {
          NEALog.errorLog(TeamDetailHeaderView.className(), desc: #function + "getTeamMember \(ownerAccountId) error: \(err.localizedDescription)")
