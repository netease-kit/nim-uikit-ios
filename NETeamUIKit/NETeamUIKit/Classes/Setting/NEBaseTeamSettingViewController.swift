    ...
    viewModel.getTeamWithMembers(tid) { error in
      ...
      if let err = error {
        ...
        weakSelf?.viewModel.getAllTeamMemberInfos(tid, .TEAM_MEMBER_ROLE_QUERY_TYPE_ALL) { error in
          NEALog.infoLog(NEBaseTeamSettingViewController.className(), desc: "CALLBACK getAllTeamMemberInfos \(error?.localizedDescription ?? "no error")")
        }
      }
    }
  }
    ...
    if let tid = teamId {
      ...
      Router.shared.use(
        ...
        parameters: ["nav": navigationController as Any,
                     ...
                     "teamInfo": viewModel.teamInfoModel as Any],
        closure: nil
      )
    }
  }
