...
由于podspec中无法通过路径来依赖本地的pod库，所以，需要在对应 Demo 的 Podfile 中添加对NETeamUIKit的依赖。

```
pod 'NETeamUIKit', :path => '../NETeamUIKit/NETeamUIKit.podspec'
```
### 界面工程直接引用
如果是example直接依赖NETeamUIKit，则直接在对应 Demo 的 Podfile 中添加对NETeamUIKit的依赖。
...
```bash
python3 xkit_im_push.py --product im -k NETeamUIKit --dry-run
python3 xkit_im_push.py --product im -k NETeamUIKit --prepare
```
