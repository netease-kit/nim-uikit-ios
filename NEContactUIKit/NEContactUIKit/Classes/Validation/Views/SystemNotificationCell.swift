...
@objcMembers
public class SystemNotificationCell: BaseValidationCell {
  ...
  override public func confige(_ model: XNotification) {
    super.confige(model)
    notifModel = model
  ...
  func rejectClick(_ sender: UIButton) {
    if let model = notifModel {
      delegate?.onRefuse(model)
    }
  }

  func agreeClick(_ sender: UIButton) {
    if let model = notifModel {
