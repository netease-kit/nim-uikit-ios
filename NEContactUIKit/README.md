...
由于podspec中无法通过路径来依赖本地的pod库，所以，需要在对应 Demo 的 Podfile 中添加对NEContactUIKit的依赖。

```
pod 'NEContactUIKit', :path => '../NEContactUIKit/NEContactUIKit.podspec'
```
### 界面工程直接引用
如果是example直接依赖NEContactUIKit，则直接在对应 Demo 的 Podfile 中添加对NEContactUIKit的依赖。
...
```bash
python3 xkit_im_push.py --product im -k NEContactUIKit --dry-run
python3 xkit_im_push.py --product im -k NEContactUIKit --prepare
```
