...
class NEHomeServerCell: UITableViewCell {
    ...
    public var serverModel:QChatServer? {
        didSet {
            if let imageUrl = serverModel?.icon {
                headView.sd_setImage(with: URL.init(string: imageUrl), completed: nil)
                headView.setTitle("")
            }else {
                if let name = serverModel?.name {
                    headView.setTitle(name)
                }
                headView.sd_setImage(with: URL.init(string: ""), completed: nil)
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.contentView.backgroundColor = HexRGB(0xe9eff5)
        setupSubviews()
    }
    ...
    func setupSubviews() {
        self.contentView.addSubview(leftSelectView)
        self.contentView.addSubview(headView)
        
        NSLayoutConstraint.activate([
            headView.leftAnchor.constraint(equalTo: self.contentView.leftAnchor, constant: 12),
            headView.topAnchor.constraint(equalTo: self.contentView.topAnchor),
            headView.widthAnchor.constraint(equalToConstant: 42),
            headView.heightAnchor.constraint(equalToConstant: 42),
        ])
        
        NSLayoutConstraint.activate([
            leftSelectView.leftAnchor.constraint(equalTo: self.contentView.leftAnchor,constant: -4),
            leftSelectView.topAnchor.constraint(equalTo: self.contentView.topAnchor),
    ...
    public func showSelectState(isShow:Bool) {
        self.leftSelectView.isHidden = isShow ? false : true
    }
