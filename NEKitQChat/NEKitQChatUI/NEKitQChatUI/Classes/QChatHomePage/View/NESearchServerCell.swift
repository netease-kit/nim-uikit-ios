...
class NESearchServerCell: UITableViewCell {
    ...
    public var serverModel:QChatServer? {
        didSet {
            if let imageUrl = serverModel?.icon {
                headImageView.sd_setImage(with: URL.init(string: imageUrl), completed: nil)
                headImageView.setTitle("")
            }else {
                if let name = serverModel?.name {
                    headImageView.setTitle(name)
                }
                headImageView.sd_setImage(with: URL.init(string: ""), completed: nil)
    ...
    func setupSubviews() {
        self.contentView.addSubview(serviceBgView)
        ...
        NSLayoutConstraint.activate([
            serviceBgView.leftAnchor.constraint(equalTo: self.contentView.leftAnchor, constant: kScreenInterval),
            serviceBgView.rightAnchor.constraint(equalTo: self.contentView.rightAnchor, constant: -kScreenInterval),
            serviceBgView.topAnchor.constraint(equalTo: self.contentView.topAnchor),
            serviceBgView.bottomAnchor.constraint(equalTo: self.contentView.bottomAnchor, constant: -6),
        ])
        ...
        NSLayoutConstraint.activate([
            ...
            rightContentImageView.rightAnchor.constraint(equalTo: serviceBgView.rightAnchor,constant: 16),
        ])
...
//        view.clipsToBounds = true
    ...
    private lazy var rightContentImageView:UIImageView = {
        let imageView = UIImageView.init(image: UIImage.ne_imageNamed(name: "addOther_icon"))
