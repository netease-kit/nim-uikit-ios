...
extension QChatPermissionViewController: UITableViewDelegate, UITableViewDataSource, QChatTextEditCellDelegate, ViewModelDelegate, QChatSwitchCellDelegate {
    
    func didChangeSwitchValue(_ cell: QChatSwitchCell) {
        print("did change switch value : ", cell)
        if let key = cell.model?.permissionKey, let value = cell.model?.permission?.value(forKey: key) as? String, let type = ChatPermissionType(rawValue: value) {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 2 || indexPath.section == 3 || indexPath.section == 4 {
            let cell: QChatSwitchCell = tableView.dequeueReusableCell(withIdentifier: "\(QChatSwitchCell.self)", for: indexPath) as! QChatSwitchCell
...
extension QChatPermissionViewController{
    ...
    func updatePermission(_ type: ChatPermissionType, _ open: Bool, _ completion: @escaping (Bool) -> Void){
