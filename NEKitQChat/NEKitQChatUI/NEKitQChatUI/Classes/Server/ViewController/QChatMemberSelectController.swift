...
//    var selectType =  SelectType.ServerMember
    ...
    func setupUI(){
        ...
        self.title = localizable("qchat_select")
        ...
        collection.register(QChatUserUnCheckCell.self, forCellWithReuseIdentifier: "\(NSStringFromClass(QChatUserUnCheckCell.self))")
        ...
        tableView.register(QChatSelectedCell.self, forCellReuseIdentifier: "\(QChatSelectedCell.self)")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if user.select == true {
            ...
            if selectArray.count >= limit {
                ...
                showToast("\(localizable("exceed"))\(limit)\(localizable("person"))\(localizable("limit"))")
                return
            }
            cell?.setSelect()
...
//    MARK: MemberSelectViewModelDelegate
    func filterMembers(accid: [String]?, _ filterMembers: @escaping ([String]?) -> ()) {
//        查询需要筛选的用户
        self.delegate?.filterMembers(accid: accid, filterMembers)
    }
