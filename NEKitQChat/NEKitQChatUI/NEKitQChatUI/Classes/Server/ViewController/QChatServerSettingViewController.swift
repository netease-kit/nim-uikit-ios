...
public class QChatServerSettingViewController: NEBaseTableViewController {
    ...
    lazy var serverNameInput: UITextField = {
        return getInput()
    }()
    
    lazy var serverThemeInput: UITextField = {
        return getInput()
    }()
    ...
    public override func viewDidLoad() {
        ...
        QChatLog.infoLog(className(), desc: "server id : \(server?.serverId ?? 0)" )
    ...
    func setupUI(){
        ...
        tableView.register(QChatDestructiveCell.self, forCellReuseIdentifier: "\(QChatDestructiveCell.self)")
    ...
    func addInputView(_ back: UIView, _ topView: UIView){
        ...
        NSLayoutConstraint.activate([
            ...
            serverThemeLabel.topAnchor.constraint(equalTo: serverNameInput.bottomAnchor, constant: 16),
        ])
        ...
        NSLayoutConstraint.activate([
            ...
            serverThemeInput.topAnchor.constraint(equalTo: serverNameInput.bottomAnchor, constant: 38),
        ...
        if let custom = server?.custom, let dic = getDictionaryFromJSONString(custom), let topic = dic["topic"] as? String{
        ...
        NSLayoutConstraint.activate([
            ...
            permissionLabel.topAnchor.constraint(equalTo: serverThemeInput.bottomAnchor, constant: 16)
        ])
...
extension QChatServerSettingViewController {
    ...
    func leaveServer(){
        if let serverid = server?.serverId {
            ...
            viewModel.repo.leaveServer(serverid) { error in
                weakSelf?.view.hideToastActivity()
                if let err = error {
                    QChatLog.errorLog(className, desc: "leave server error : \(err)")
...
extension QChatServerSettingViewController: UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let destructiveCell: QChatDestructiveCell = tableView.dequeueReusableCell(withIdentifier: "\(QChatDestructiveCell.self)", for: indexPath) as! QChatDestructiveCell
            
            destructiveCell.redTextLabel.text = isMyServer() ? localizable("qchat_delete_server") : localizable("qchat_leave_server")
            destructiveCell.cornerType = CornerType.bottomLeft.union(CornerType.bottomRight).union(CornerType.topLeft).union(CornerType.topRight)
...
extension QChatServerSettingViewController: UINavigationControllerDelegate {
