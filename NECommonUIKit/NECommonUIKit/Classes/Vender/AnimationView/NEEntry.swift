...
struct NEEntry: Equatable {
  // MARK: Lifecycle

  init?(centralDirectoryStructure: NECentralDirectoryStructure,
        localFileHeader: NELocalFileHeader,
  ...
  struct NELocalFileHeader: NEDataSerializable {
    ...
    var extraFields: [NEExtensibleDataField]?
  }

  struct NEDataDescriptor<T: BinaryInteger>: NEDataSerializable {
    let data: Data
  ...
  typealias DefaultDataDescriptor = NEDataDescriptor<UInt32>
  typealias ZIP64DataDescriptor = NEDataDescriptor<UInt64>

  struct NECentralDirectoryStructure: NEDataSerializable {
    static let size = 46
  ...
  let centralDirectoryStructure: NECentralDirectoryStructure
  ...
  /// The `NEEntryType` of the receiver.
  var type: NEEntryType {
  ...
  var isCompressed: Bool {
    localFileHeader.compressionMethod != NECompressionMethod.none.rawValue
  }
  ...
  var localSize: UInt64 {
    ...
    var size = UInt64(NELocalFileHeader.size + extraDataLength)
...
extension NEEntry.NECentralDirectoryStructure {
  init(localFileHeader: NEEntry.NELocalFileHeader,
  ...
  init(centralDirectoryStructure: NEEntry.NECentralDirectoryStructure,
    ...
    if let existingInfo = zip64ExtendedInformation {
      ...
      versionNeededToExtract = max(
        ...
        NEArchive.NEVersion.v45.rawValue
      )
    } else {
      ...
      versionNeededToExtract = existingVersion < NEArchive.NEVersion.v45.rawValue
        ? centralDirectoryStructure.versionNeededToExtract
        : NEArchive.NEVersion.v20.rawValue
    }
