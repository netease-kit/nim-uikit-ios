...
/// `NEAnimationKeypath` is an object that describes a keypath search for nodes in the
...
/// with an existing `NELottieAnimation`.
///
/// `NEAnimationKeypath` can describe a specific object, or can use wildcards for fuzzy matching
...
///
/// EG:
/// @"Layer.NEShape NEGroup.NEStroke 1.Color"
/// Represents a specific color node on a specific stroke.
///
/// @"**.NEStroke 1.Color"
/// Represents the color node for every NEStroke named "NEStroke 1" in the animation.
public struct NEAnimationKeypath: Hashable, ExpressibleByStringLiteral {
  // MARK: Lifecycle
