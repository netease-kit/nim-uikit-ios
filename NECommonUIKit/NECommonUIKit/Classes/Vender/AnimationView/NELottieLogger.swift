...
public final class NELottieLogger {
  // MARK: Lifecycle

  public init(assert: @escaping Assert = { condition, message, file, line in
  ...
  public typealias Assert = (_ condition: @autoclosure () -> Bool,
  ...
  public typealias AssertionFailure = (_ message: @autoclosure () -> String,
                                       _ fileID: StaticString,
                                       _ line: UInt)
    -> Void

  /// Logs a warning message.
  public typealias Warn = (_ message: @autoclosure () -> String,
                           _ fileID: StaticString,
                           _ line: UInt)
    -> Void
  ...
  public static var shared = NELottieLogger()
  ...
  /// Logs a warning message.
  public func warn(_ message: @autoclosure () -> String = String(),
                   fileID: StaticString = #fileID,
                   line: UInt = #line) {
...
// MARK: - NELottieLogger + printToConsole

public extension NELottieLogger {
  /// A `NELottieLogger` instance that always prints to the console (by calling `print`)
  /// instead of calling `assert` / `assertionFailure`, which halt execution in debug builds.
  static var printToConsole: NELottieLogger {
    NELottieLogger(
