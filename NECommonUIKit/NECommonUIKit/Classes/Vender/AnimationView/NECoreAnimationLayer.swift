...
final class NECoreAnimationLayer: NEBaseAnimationLayer {
  // MARK: Lifecycle
  ...
  init(animation: NELottieAnimation,
       ...
       compatibilityTrackerMode: NECompatibilityTracker.NEMode,
    ...
    self.logger = logger
    compatibilityTracker = NECompatibilityTracker(mode: compatibilityTrackerMode, logger: logger)
    valueProviderStore = NEValueProviderStore(logger: logger)
    super.init()
  ...
  struct NECAMediaTimingConfiguration: Equatable {
  ...
  enum NEPlaybackState: Equatable {
    ...
    case paused(frame: NEAnimationFrameTime)
  }

  /// Configuration used by the `playAnimation` method
  struct NEAnimationConfiguration: Equatable {
    var animationContext: NEAnimationContext
    var timingConfiguration: NECAMediaTimingConfiguration
    var recordHierarchyKeypath: ((String) -> Void)?

    static func == (_ lhs: NEAnimationConfiguration, _ rhs: NEAnimationConfiguration) -> Bool {
  ...
  var didSetUpAnimation: (([NECompatibilityIssue]) -> Void)?
  ...
  func playAnimation(configuration: NEAnimationConfiguration,
                     playbackState: NEPlaybackState = .playing) {
  ...
  private var pendingAnimationConfiguration: (
    animationConfiguration: NEAnimationConfiguration,
    playbackState: NEPlaybackState
  )?

  /// A modification that should be applied to the next animation configuration
  private var pendingAnimationConfigurationModification: ((inout NEAnimationConfiguration) -> Void)?

  /// Configuration for the animation that is currently setup in this layer
  private var currentAnimationConfiguration: NEAnimationConfiguration?
  ...
  private let valueProviderStore: NEValueProviderStore
  private let compatibilityTracker: NECompatibilityTracker
  private let logger: NELottieLogger
  private let loggingState = NELoggingState()

  /// The current playback state of the animation that is displayed in this layer
  private var currentPlaybackState: NEPlaybackState? {
    didSet {
  ...
  private func setupAnimation(for configuration: NEAnimationConfiguration) throws {
    // Remove any existing animations from the layer hierarchy
    removeAnimations()

    currentAnimationConfiguration = configuration

    let layerContext = NELayerAnimationContext(
      animation: animation,
      ...
      currentKeypath: NEAnimationKeypath(keys: []),
      textProvider: textProvider,
    ...
    for animationLayer in sublayers ?? [] {
      try (animationLayer as? NEAnimationLayer)?.setupAnimations(context: layerContext)
    }
  }
  ...
  private func setupPlaceholderAnimation(context: NELayerAnimationContext) {
    ...
    let timedProgressAnimation = animationProgressTracker.neTimed(with: context, for: self)
...
extension NECoreAnimationLayer: NERootAnimationLayer {
  ...
  var currentFrame: NEAnimationFrameTime {
    get {
      switch playbackState {
      case let .paused(frame):
        return frame
    ...
    set {
      ...
      let requiredAnimationConfiguration = NEAnimationConfiguration(
        animationContext: NEAnimationContext(
        ...
        timingConfiguration: NECAMediaTimingConfiguration(speed: 0)
      )
  ...
  var _animationLayers: [CALayer] {
    (sublayers ?? []).filter { $0 is NEAnimationLayer }
  }
  ...
  func reloadImages() {
    ...
    for sublayer in allSublayers {
      if let imageLayer = sublayer as? NEImageLayer {
  ...
  func reloadFonts() {
    ...
    for sublayer in allSublayers {
      if let textLayer = sublayer as? NETextLayer {
  ...
  func getValue(for _: NEAnimationKeypath, atFrame _: NEAnimationFrameTime?) -> Any? {
    logger.assertionFailure("""
  ...
  func getOriginalValue(for _: NEAnimationKeypath, atFrame _: NEAnimationFrameTime?) -> Any? {
    logger.assertionFailure("""
    The Core Animation rendering engine doesn't support querying values for individual frames
    """)
    return nil
  ...
  func layer(for _: NEAnimationKeypath) -> CALayer? {
    logger.assertionFailure("""
  ...
  func animatorNodes(for _: NEAnimationKeypath) -> [NEAnimatorNode]? {
    logger.assertionFailure("""
...
extension CALayer {
  ...
  var numberOfLayersWithTimeRemapping: Int {
    ...
    for sublayer in sublayers ?? [] {
      if
        let preCompLayer = sublayer as? NEPreCompLayer,
