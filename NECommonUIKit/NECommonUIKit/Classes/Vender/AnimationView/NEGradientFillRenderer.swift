...
// MARK: - NEGradientFillLayer

private final class NEGradientFillLayer: CALayer {
  ...
  override func draw(in ctx: CGContext) {
    ...
    for i in 0 ..< numberOfColors {
      let ix = i * 4
      if colors.count > ix {
        let color = CGColor.neRgb(colors[ix + 1], colors[ix + 2], colors[ix + 3])
    ...
    for i in stride(from: numberOfColors * 4, to: colors.endIndex, by: 2) {
      ...
      alphaColors.append(.neGray(alpha))
    }
    ...
    guard
      let gradient = CGGradient(
        colorsSpace: NELottieConfiguration.shared.colorSpace,
...
// MARK: - NEGradientFillRenderer

/// A rendered for a Path NEFill
final class NEGradientFillRenderer: NEPassThroughOutputNode, NERenderable {
  // MARK: Lifecycle

  override init(parent: NENodeOutput?) {
    super.init(parent: parent)

    maskLayer.fillColor = .neRgb(1, 1, 1)
  ...
  private let gradientLayer = NEGradientFillLayer()
