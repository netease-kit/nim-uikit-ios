...
struct NEBezierPath {
  ...
  init(startPoint: NECurveVertex) {
    elements = [NEPathElement(vertex: startPoint)]
    length = 0
    closed = false
  }
  ...
  private(set) var elements: [NEPathElement]
  ...
  mutating func moveToStartPoint(_ vertex: NECurveVertex) {
    elements = [NEPathElement(vertex: vertex)]
    length = 0
  }

  mutating func addVertex(_ vertex: NECurveVertex) {
    guard let previous = elements.last else {
      addElement(NEPathElement(vertex: vertex))
      return
    }
  ...
  mutating func addCurve(toPoint: CGPoint, outTangent: CGPoint, inTangent: CGPoint) {
    ...
    let newVertex = NECurveVertex(inTangent, toPoint, toPoint)
    updateVertex(
      NECurveVertex(previous.vertex.inTangent, previous.vertex.point, outTangent),
  ...
  mutating func addLine(toPoint: CGPoint) {
    guard let previous = elements.last else { return }
    let newVertex = NECurveVertex(point: toPoint, inTangentRelative: .zero, outTangentRelative: .zero)
    updateVertex(
      NECurveVertex(previous.vertex.inTangent, previous.vertex.point, previous.vertex.point),
      atIndex: elements.endIndex - 1,
      remeasure: false
    )
    addVertex(newVertex)
  ...
  mutating func addElement(_ pathElement: NEPathElement) {
    elements.append(pathElement)
    length = length + pathElement.length
  }

  mutating func updateVertex(_ vertex: NECurveVertex, atIndex: Int, remeasure: Bool) {
    if remeasure {
      var newElement: NEPathElement
      if atIndex > 0 {
        let previousElement = elements[atIndex - 1]
        newElement = previousElement.pathElementTo(vertex)
      } else {
        newElement = NEPathElement(vertex: vertex)
      }
  ...
  ///
  func trim(fromLength: CGFloat, toLength: CGFloat, offsetLength: CGFloat) -> [NEBezierPath] {
  ...
  private func trimPathAtLengths(positions: [(start: CGFloat, end: CGFloat)]) -> [NEBezierPath] {
...
extension NEBezierPath: Codable {
  // MARK: Lifecycle

  init(from decoder: Decoder) throws {
    let container: KeyedDecodingContainer<NEBezierPath.NECodingKeys>

    if let keyedContainer = try? decoder.container(keyedBy: NEBezierPath.NECodingKeys.self) {
      container = keyedContainer
    } else {
      var unkeyedContainer = try decoder.unkeyedContainer()
      container = try unkeyedContainer.nestedContainer(keyedBy: NEBezierPath.NECodingKeys.self)
    }
    ...
    var decodedElements = [NEPathElement]()

    /// Create first point
    let firstVertex = try NECurveVertex(
  ...
  /// The NEBezierPath container is encoded and decoded from the JSON format
  ...
  func encode(to encoder: Encoder) throws {
    var container = encoder.container(keyedBy: NEBezierPath.NECodingKeys.self)
...
extension NEBezierPath: NEAnyInitializable {
  init(value: Any) throws {
    ...
    closed = (try? pathDictionary.value(for: NECodingKeys.closed)) ?? true
    var vertexDictionaries: [Any] = try pathDictionary.value(for: NECodingKeys.vertices)
    var inPointsDictionaries: [Any] = try pathDictionary.value(for: NECodingKeys.inPoints)
    var outPointsDictionaries: [Any] = try pathDictionary.value(for: NECodingKeys.outPoints)
    guard
...
extension NEBezierPath {
  func cgPath() -> CGPath {
    ...
    var previousElement: NEPathElement?
