...
extension CALayer {
  ...
  @nonobjc
  func neAddAnimation<KeyframeValue: NEAnyInterpolatable, ValueRepresentation>(for property: NELayerProperty<ValueRepresentation>,
                                                                               keyframes: NEKeyframeGroup<KeyframeValue>,
                                                                               value keyframeValueMapping: (KeyframeValue) throws -> ValueRepresentation,
                                                                               context: NELayerAnimationContext)
    throws {
    if let customAnimation = try neCustomizedAnimation(for: property, context: context) {
      neAdd(customAnimation, timedWith: context)
    }

    else if
      let defaultAnimation = try neDefaultAnimation(
        for: property,
        keyframes: keyframes,
        value: keyframeValueMapping,
        context: context
      ) {
      let timedAnimation = defaultAnimation.neTimed(with: context, for: self)
  ...
  @nonobjc
  private func neDefaultAnimation<KeyframeValue: NEAnyInterpolatable, ValueRepresentation>(for property: NELayerProperty<ValueRepresentation>,
                                                                                           keyframes keyframeGroup: NEKeyframeGroup<KeyframeValue>,
                                                                                           value keyframeValueMapping: (KeyframeValue) throws -> ValueRepresentation,
                                                                                           context: NELayerAnimationContext)
    throws -> CAAnimation? {
    ...
    //  - We only log this once per `NECoreAnimationLayer` instance.
    ...
    if keyframes.count == 1 {
      return try neSingleKeyframeAnimation(
        for: property,
    ...
    /// If we're required to use the `complexTimeRemapping` from some parent `NEPreCompLayer`,
    /// we have to manually interpolate the keyframes with the time remapping applied.
    if context.mustUseComplexTimeRemapping {
      return try neDefaultAnimation(
        for: property,
        keyframes: NEKeyframes.manuallyInterpolatedWithTimeRemapping(keyframeGroup, context: context),
        value: keyframeValueMapping,
    ...
    let animationSegments = keyframes.neSegmentsSplitByCalculationMode()
    ...
    if animationSegments.count == 1 {
      return try neKeyframeAnimation(
        for: property,
  ...
  /// A `CAAnimation` that applies the custom value from the `NEAnyValueProvider`
  /// registered for this specific property's `NEAnimationKeypath`,
  /// if one has been registered using `NELottieAnimationView.setValueProvider(_:keypath:)`.
  @nonobjc
  private func neCustomizedAnimation<ValueRepresentation>(for property: NELayerProperty<ValueRepresentation>,
                                                          context: NELayerAnimationContext)
    ...
    guard
      ...
      let customKeyframes = try context.valueProviderStore.customKeyframes(
        ...
        for: NEAnimationKeypath(keys: context.currentKeypath.keys + customizableProperty.name.map { $0.rawValue }),
        context: context
      )
    else { return nil }
    ...
    if
      ...
      let singleKeyframeAnimation = neSingleKeyframeAnimation(
        for: property,
  ...
  private func neSingleKeyframeAnimation<ValueRepresentation>(for property: NELayerProperty<ValueRepresentation>,
  ...
  private func neAnimationGroup<KeyframeValue, ValueRepresentation>(for property: NELayerProperty<ValueRepresentation>,
                                                                    animationSegments: [[NEKeyframe<KeyframeValue>]],
                                                                    value keyframeValueMapping: (KeyframeValue) throws -> ValueRepresentation,
                                                                    context: NELayerAnimationContext)
    ...
    let segmentAnimations: [CAKeyframeAnimation] = try animationSegments.indices.map { index in
      ...
      if isFirstSegment {
        segmentStartTime = try min(
          context.time(forFrame: context.animation.startFrame),
          segmentStartTime
        )
      }

      if isLastSegment {
        segmentEndTime = try max(
          context.time(forFrame: context.animation.endFrame),
          segmentEndTime
        )
      }
      ...
      let animation = try neKeyframeAnimation(
        for: property,
  ...
  private func neKeyframeAnimation<KeyframeValue, ValueRepresentation>(for property: NELayerProperty<ValueRepresentation>,
                                                                       keyframes: [NEKeyframe<KeyframeValue>],
                                                                       value keyframeValueMapping: (KeyframeValue) throws -> ValueRepresentation,
                                                                       customKeyTimes: [NSNumber]? = nil,
                                                                       context: NELayerAnimationContext)
    throws
    -> CAKeyframeAnimation {
    // Convert the list of `NEKeyframe<T>` into
    // the representation used by `CAKeyframeAnimation`
    var keyTimes = try customKeyTimes ?? keyframes.map { keyframeModel -> NSNumber in
      try NSNumber(value: Float(context.progressTime(for: keyframeModel.time)))
    }

    var timingFunctions = neTimingFunctions(for: keyframes)
    let calculationMode = neCalculationMode(for: keyframes)
    ...
    if property.caLayerKeypath == NELayerProperty<CGPoint>.position.caLayerKeypath {
      animation.path = try nePath(keyframes: keyframes, value: { value in
  ...
  private func neCalculationMode<KeyframeValue>(
    for keyframes: [NEKeyframe<KeyframeValue>])
  ...
  private func neTimingFunctions<KeyframeValue>(
    for keyframes: [NEKeyframe<KeyframeValue>])
  ...
  private func nePath<KeyframeValue>(keyframes positionKeyframes: [NEKeyframe<KeyframeValue>],
  ...
  private func neValidate<ValueRepresentation>(values: inout [ValueRepresentation],
    ...
    case .linear, .cubic:
      ...
      context.logger.assert(
        ...
        "`neTimingFunctions.count` must exactly equal `values.count - 1`"
      )

    case .discrete:
      // From the documentation of `CAKeyframeAnimation.keyTimes`:
      //  - If the neCalculationMode is set to discrete... the keyTimes array
...
extension RandomAccessCollection {
  /// Splits this array of `NEKeyframe`s into segments with the same `CAAnimationCalculationMode`
  ///  - NEKeyframes with `isHold=true` become `discrete`, and keyframes with `isHold=false`
  ...
  func neSegmentsSplitByCalculationMode<KeyframeValue>() -> [[Element]]
    where Element == NEKeyframe<KeyframeValue>, Index == Int {
