...
extension NEArchive {
  ...
  func readUncompressed(entry: NEEntry,
                        bufferSize: Int,
                        skipCRC32: Bool,
                        progress: Progress? = nil,
                        with consumer: NEConsumer)
    throws -> NECRC32 {
    let size = entry.centralDirectoryStructure.effectiveUncompressedSize
    guard size <= .max else { throw NEArchiveError.invalidEntrySize }
    return try Data.neConsumePart(
      of: Int64(size),
  ...
  func readCompressed(entry: NEEntry,
                      bufferSize: Int,
                      skipCRC32: Bool,
                      progress: Progress? = nil,
                      with consumer: NEConsumer)
  ...
  func writeEntry(uncompressedSize: Int64,
                  type: NEEntry.NEEntryType,
                  compressionMethod: NECompressionMethod,
                  bufferSize: Int,
                  progress: Progress? = nil,
                  provider: NEProvider) throws -> (sizeWritten: Int64, crc32: NECRC32) {
  ...
  func writeLocalFileHeader(path: String,
    ...
    var versionNeededToExtract = NEVersion.v20.rawValue
    ...
    if size.uncompressed >= maxUncompressedSize || size.compressed >= maxCompressedSize {
      ...
      versionNeededToExtract = NEVersion.v45.rawValue
      zip64ExtendedInformation = NEEntry.ZIP64ExtendedInformation(
    ...
    let localFileHeader = NELocalFileHeader(
    ...
    _ = try Data.neWrite(chunk: localFileHeader.data, to: archiveFile)
    return localFileHeader
  }

  func writeCentralDirectoryStructure(localFileHeader: NELocalFileHeader,
                                      relativeOffset: UInt64,
                                      externalFileAttributes: UInt32)
    throws -> NECentralDirectoryStructure {
    ...
    if localFileHeader.uncompressedSize == .max || localFileHeader.compressedSize == .max {
      let zip64Field = NEEntry.ZIP64ExtendedInformation
    ...
    if extraFieldLength > 0 {
      ...
      extraFieldLength += NEEntry.ZIP64ExtendedInformation.headerSize
    }
    let centralDirectory = NECentralDirectoryStructure(
    ...
    _ = try Data.neWrite(chunk: centralDirectory.data, to: archiveFile)
    return centralDirectory
  }

  func writeEndOfCentralDirectory(centralDirectoryStructure: NECentralDirectoryStructure,
                                  startOfCentralDirectory: UInt64,
                                  startOfEndOfCentralDirectory: UInt64,
                                  operation: NEModifyOperation)
    throws -> NEEndOfCentralDirectoryStructure {
    ...
    let cdDataLengthChange = countChange * (Int(dataLength) + NECentralDirectoryStructure.size)
    let (updatedSizeOfCD, updatedNumberOfEntries): (UInt64, UInt64) = try {
      switch operation {
      case .add:
        guard .max - sizeOfCD >= cdDataLengthChange else {
          throw NEArchiveError.invalidCentralDirectorySize
        }
        guard .max - numberOfTotalEntries >= countChange else {
          throw NEArchiveError.invalidCentralDirectoryEntryCount
        }
    ...
    record = NEEndOfCentralDirectoryRecord(
      record: record,
    ...
    _ = try Data.neWrite(chunk: record.data, to: archiveFile)
    return (record, zip64EOCD)
  }

  func writeUncompressed(size: Int64,
                         bufferSize: Int,
                         progress: Progress? = nil,
                         provider: NEProvider) throws -> (sizeWritten: Int64, checksum: NECRC32) {
    ...
    while position < size {
      ...
      checksum = entryChunk.neCrc32(checksum: checksum)
      try sizeWritten += Int64(Data.neWrite(chunk: entryChunk, to: archiveFile))
  ...
  func writeCompressed(size: Int64,
                       bufferSize: Int,
                       progress: Progress? = nil,
                       provider: NEProvider) throws -> (sizeWritten: Int64, checksum: NECRC32) {
    var sizeWritten: Int64 = 0
    let consumer: NEConsumer = { data in try sizeWritten += Int64(Data.neWrite(chunk: data, to: self.archiveFile)) }
    let checksum = try Data.neCompress(
      size: size,
      bufferSize: bufferSize,
  ...
  func writeSymbolicLink(size: Int, provider: NEProvider) throws -> (sizeWritten: Int, checksum: NECRC32) {
    // The reported size of a symlink is the number of characters in the path it points to.
    let linkData = try provider(0, size)
    let checksum = linkData.neCrc32(checksum: 0)
    let sizeWritten = try Data.neWrite(chunk: linkData, to: archiveFile)
    return (sizeWritten, checksum)
  }
