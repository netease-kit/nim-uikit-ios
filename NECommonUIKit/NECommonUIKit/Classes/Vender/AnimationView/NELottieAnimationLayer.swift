...
// MARK: - NELottieAnimationLayer

/// A CALayer subclass for rendering Lottie animations.
///  - Also available as a SwiftUI view (`NELottieView`) and a UIView subclass (`NELottieAnimationView`)
public class NELottieAnimationLayer: CALayer {
  // MARK: Lifecycle

  /// Initializes a NELottieAnimationLayer with an animation.
  public init(animation: NELottieAnimation?,
              imageProvider: NEAnimationImageProvider? = nil,
              textProvider: NEAnimationKeypathTextProvider = NEDefaultTextProvider(),
              fontProvider: NEAnimationFontProvider = NEDefaultFontProvider(),
              configuration: NELottieConfiguration = .shared,
              logger: NELottieLogger = .shared) {
    self.animation = animation
    self.imageProvider = imageProvider ?? NEBundleImageProvider(bundle: Bundle.main, searchPath: nil)
  ...
  /// Initializes an NELottieAnimationLayer with a .lottie file.
  public init(dotLottie: NEDotLottieFile?,
    ...
    imageProvider = dotLottie?.imageProvider ?? NEBundleImageProvider(bundle: Bundle.main, searchPath: nil)
    self.textProvider = textProvider
    self.fontProvider = fontProvider
    self.configuration = configuration
    screenScale = 1
  ...
  public init(configuration: NELottieConfiguration = .shared,
              logger: NELottieLogger = .shared) {
    animation = nil
    imageProvider = NEBundleImageProvider(bundle: Bundle.main, searchPath: nil)
  ...
  open func play(completion: NELottieCompletionBlock? = nil) {
    ...
    let context = NEAnimationContext(
  ...
  open func play(fromProgress: NEAnimationProgressTime? = nil,
                 toProgress: NEAnimationProgressTime,
                 loopMode: NELottieLoopMode? = nil,
                 completion: NELottieCompletionBlock? = nil) {
    guard let animation else { return }
  ...
  /// - Parameter loopMode: The loop behavior of the animation. If `nil` the layer's `loopMode` property will be used.
  /// - Parameter completion: An optional completion closure to be called when the animation stops.
  open func play(fromFrame: NEAnimationFrameTime? = nil,
  ...
  open func pause(at state: NELottiePlaybackMode.PausedState) {
    switch state {
    case .currentFrame:
      removeCurrentAnimation()
  ...
  open func play(_ playbackMode: NELottiePlaybackMode,
                 animationCompletionHandler: NELottieCompletionBlock? = nil) {
    setPlaybackMode(playbackMode, completion: animationCompletionHandler)
  }

  /// Applies the given `NELottiePlaybackMode` to this layer.
  /// - Parameter NEPlaybackMode: The playback mode to apply
  /// - Parameter completion: A closure that is called after
  ///   an animation triggered by this method completes.
  open func setPlaybackMode(_ playbackMode: NELottiePlaybackMode,
                            completion: NELottieCompletionBlock? = nil) {
    switch playbackMode {
  ...
  /// - Parameter completion: A closure that is called after
  ///   an animation triggered by this method completes.
  open func play(_ playbackMode: NELottiePlaybackMode.PlaybackMode, completion: NELottieCompletionBlock? = nil) {
    switch playbackMode {
    case let .fromProgress(from, to, loopMode):
      play(
        fromProgress: from,
  ...
  /// The current `NELottiePlaybackMode` that is being used
  public private(set) var currentPlaybackMode: NELottiePlaybackMode?

  /// Value Providers that have been registered using `setValueProvider(_:keypath:)`
  public private(set) var valueProviders = [NEAnimationKeypath: NEAnyValueProvider]()
  ...
  public var animationLayerDidLoad: ((_ animationLayer: NELottieAnimationLayer, _ renderingEngine: NERenderingEngineOption) -> Void)?

  /// The configuration that this `NELottieAnimationView` uses when playing its animation
  public var configuration: NELottieConfiguration {
    didSet {
  ...
  public var backgroundBehavior: NELottieBackgroundBehavior {
    get {
      ...
      if
        ...
        `NELottieBackgroundBehavior.continuePlaying` should not be used with the Main Thread
  ...
  ///
  /// When initializing a `NELottieAnimationView`, the animation will either be loaded
  /// synchronously (when loading a `NELottieAnimation` from a .json file on disk)
  /// or asynchronously (when loading a `NEDotLottieFile` from disk, or downloading
  ...
  /// `NELottieAnimationView` regardless of which initializer was used. For example:
  ///
  /// ```
  /// let animationView: NELottieAnimationView
  ///
  /// if loadDotLottieFile {
  ///   // Loads the .lottie file asynchronously
  ///   animationView = NELottieAnimationView(dotLottieName: "animation")
  /// } else {
  ///   // Loads the .json file synchronously
  ///   animationView = NELottieAnimationView(name: "animation")
  /// }
  ///
  ...
  /// }
  /// ```
  public var animationLoaded: ((_ animationLayer: NELottieAnimationLayer, _ animation: NELottieAnimation) -> Void)? {
    didSet {
      if let animation {
        animationLoaded?(self, animation)
      }
  ...
  public var textProvider: NEAnimationKeypathTextProvider {
    didSet {
  ...
  /// Sets the text provider for animation layer. A text provider provides the
  /// animation with values for text layers
  public var fontProvider: NEAnimationFontProvider {
    didSet {
  ...
  public var loopMode: NELottieLoopMode = .playOnce {
    didSet {
  ...
  public var currentProgress: NEAnimationProgressTime {
    set {
      if let animation {
  ...
  ///
  /// Note: Setting this will stop the current animation, if any.
  public var currentFrame: NEAnimationFrameTime {
    set {
      removeCurrentAnimationIfNecessary()
  ...
  public var realtimeAnimationFrame: NEAnimationFrameTime {
    isAnimationPlaying ? rootAnimationLayer?.presentation()?.currentFrame ?? currentFrame : currentFrame
  }

  /// Returns the current animation frame while an animation is playing.
  public var realtimeAnimationProgress: NEAnimationProgressTime {
    if let animation {
  ...
  /// `NELottieAnimation` model. When `false` the animation will play at the framerate
  /// of the device.
  ///
  /// Defaults to false
  ...
  ///    but a `NERootAnimationLayer` hasn't been constructed yet
  public var currentRenderingEngine: NERenderingEngine? {
    ...
    case .automatic:
      ...
      if animationLayer is NECoreAnimationLayer {
  ...
  public var mainThreadRenderingEngineShouldForceDisplayUpdateOnEachFrame = false {
    didSet {
      (rootAnimationLayer as? NEMainThreadAnimationLayer)?.forceDisplayUpdateOnEachFrame
  ...
  public func loadAnimation(_ animationId: String? = nil,
                            from dotLottieFile: NEDotLottieFile) {
  ...
  ///   Defaults to play first animation in file.
  ///   - dotLottieFile: Lottie file to play
  public func loadAnimation(atIndex index: Int,
                            from dotLottieFile: NEDotLottieFile) {
  ...
  /// Forces the NELottieAnimationView to redraw its contents.
  public func forceDisplayUpdate() {
    rootAnimationLayer?.forceDisplayUpdate()
  }

  /// Sets a NEValueProvider for the specified keypath. The value provider will be set
  ...
  /// Setting a NEValueProvider on a keypath will cause the animation to update its
  /// contents and read the new Value NEProvider.
  ///
  ...
  /// Example:
  /// ```
  /// /// A keypath that finds the color value for all `NEFill 1` nodes.
  /// let fillKeypath = NEAnimationKeypath(keypath: "**.NEFill 1.Color")
  /// /// A Color Value provider that returns a reddish color.
  /// let redValueProvider = NEColorValueProvider(Color(r: 1, g: 0.2, b: 0.3, a: 1))
  ...
  public func getValue(for keypath: NEAnimationKeypath, atFrame: NEAnimationFrameTime?) -> Any? {
  ...
  /// - Parameter for: The keypath used to search for the property.
  /// - Parameter atFrame: The Frame Time of the value to query. If nil then the current frame is used.
  public func getOriginalValue(for keypath: NEAnimationKeypath, atFrame: NEAnimationFrameTime?) -> Any? {
  ...
  /// Converts a CGRect from the NELottieAnimationView's coordinate space into the
  ...
  public func convert(_ rect: CGRect, toLayerAt keypath: NEAnimationKeypath?) -> CGRect? {
  ...
  /// Converts a CGPoint from the NELottieAnimationView's coordinate space into the
  /// coordinate space of the layer found at Keypath.
  ///
  /// If no layer is found, nil is returned
  ///
  ...
  /// - Parameter toLayerAt: The keypath used to find the layer.
  public func convert(_ point: CGPoint, toLayerAt keypath: NEAnimationKeypath?) -> CGPoint? {
    guard let animationLayer = rootAnimationLayer else { return nil }
    guard let keypath else {
  ...
  public func setNodeIsEnabled(isEnabled: Bool, keypath: NEAnimationKeypath) {
    guard let animationLayer = rootAnimationLayer else { return }
  ...
  public func progressTime(forMarker named: String) -> NEAnimationProgressTime? {
  ...
  public func frameTime(forMarker named: String) -> NEAnimationFrameTime? {
    guard let animation else {
      return nil
    }
  ...
  public func durationFrameTime(forMarker named: String) -> NEAnimationFrameTime? {
    guard let animation else {
      return nil
    }
  ...
  fileprivate(set) var animationContext: NEAnimationContext?
  ...
  var animationName: String? {
    didSet {
      animation = animationName.flatMap {
        NELottieAnimation.named($0, animationCache: nil)
      }
    }
  }
  ...
  func updateAnimationFrame(_ newFrame: CGFloat) {
    ...
    if NETestHelpers.performanceTestsAreRunning {
  ...
  func updateInFlightAnimation() {
    ...
    let newContext = NEAnimationContext(
  ...
  func loadAnimation(_ animationSource: NELottieAnimationSource?) {
  ...
  fileprivate var _activeAnimationName: String = NELottieAnimationLayer.animationName
  ...
  fileprivate func makeAnimationLayer(usingEngine renderingEngine: NERenderingEngineOption) {
    ...
    let rootAnimationLayer: NERootAnimationLayer?
  ...
  fileprivate func makeMainThreadAnimationLayer(for animation: NELottieAnimation) -> NEMainThreadAnimationLayer {
    let mainThreadAnimationLayer = NEMainThreadAnimationLayer(
      animation: animation,
  ...
  fileprivate func makeCoreAnimationLayer(for animation: NELottieAnimation) -> NECoreAnimationLayer? {
    do {
      let coreAnimationLayer = try NECoreAnimationLayer(
        animation: animation,
        imageProvider: imageProvider.cachedImageProvider,
        textProvider: textProvider,
        fontProvider: fontProvider,
      ...
      coreAnimationLayer.didSetUpAnimation = { [logger] compatibilityIssues in
        logger.assert(
          ...
          "Encountered Core Animation compatibility issues while setting up animation:\n"
            ...
            To resolve this issue, you can use `NERenderingEngineOption.automatic`, which automatically falls back
            to the Main Thread rendering engine when necessary, or just use `NERenderingEngineOption.mainThread`.
      ...
      // This should never happen, because we initialize the `NECoreAnimationLayer` with
      // `NECompatibilityTracker.Mode.track` (which reports errors in `didSetUpAnimation`,
      // not by throwing).
  ...
  fileprivate func makeAutomaticEngineLayer(for animation: NELottieAnimation) -> NECoreAnimationLayer? {
    do {
      ...
      if case let NECompatibilityTracker.Error.encounteredCompatibilityIssue(compatibilityIssue) = error {
        automaticEngineLayerDidSetUpAnimation([compatibilityIssue])
      } else {
        // This should never happen, because we expect `NECoreAnimationLayer` to only throw
        // `NECompatibilityTracker.Error.encounteredCompatibilityIssue` errors.
        logger.assertionFailure("Encountered unexpected error \(error)")
  ...
  fileprivate func automaticEngineLayerDidSetUpAnimation(_ compatibilityIssues: [NECompatibilityIssue]) {
    ...
    // Disable the completion handler delegate before tearing down the `NECoreAnimationLayer`
    // and building the `NEMainThreadAnimationLayer`. Otherwise deinitializing the
    // `NECoreAnimationLayer` would trigger the animation completion handler even though
    ...
    if let animationContext {
      // `NEAnimationContext.closure` (`NEAnimationCompletionDelegate`) is a reference type
      ...
      addNewAnimationForContext(NEAnimationContext(
        playFrom: animationContext.playFrom,
        playTo: animationContext.playTo,
        closure: animationContext.closure.completionBlock
      ))
  ...
  ///    animation -- otherwise this NELottieAnimationView could be put in an inconsistent state.
  fileprivate func removeCurrentAnimationIfNecessary() {
    ...
    case .coreAnimation, nil:
      ...
      // `NELottieAnimationView` in an inconsistent state.
      animationContext = nil
    }
  }

  /// Adds animation to animation layer and sets the delegate. If animation layer or animation are nil, exits.
  fileprivate func addNewAnimationForContext(_ animationContext: NEAnimationContext) {
    ...
    _activeAnimationName = NELottieAnimationLayer.animationName + String(animationID)

    if let coreAnimationLayer = animationlayer as? NECoreAnimationLayer {
      ...
      var timingConfiguration = NECoreAnimationLayer.NECAMediaTimingConfiguration(
        autoreverses: loopMode.caNEAnimationConfiguration.autoreverses,
        repeatCount: loopMode.caNEAnimationConfiguration.repeatCount,
    ...
    layerAnimation.repeatCount = loopMode.caNEAnimationConfiguration.repeatCount
    layerAnimation.autoreverses = loopMode.caNEAnimationConfiguration.autoreverses
  ...
  /// The `NELottieBackgroundBehavior` that was specified manually by setting `self.backgroundBehavior`
  private var _backgroundBehavior: NELottieBackgroundBehavior?
  ...
  private var reducedMotionMarker: NEMarker? {
  ...
  private func loadAnimation(_ dotLottieAnimation: NEDotLottieFile.Animation) {
  ...
  private func playReducedMotionAnimation(completion: NELottieCompletionBlock?) {
...
// MARK: - NELottieLoopMode + caNEAnimationConfiguration

extension NELottieLoopMode {
  /// The `CAAnimation` configuration that reflects this mode
  var caNEAnimationConfiguration: (repeatCount: Float, autoreverses: Bool) {
    switch self {
    case .playOnce:
