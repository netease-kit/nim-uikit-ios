...
// MARK: - NELottieBackgroundBehavior
...
public enum NELottieBackgroundBehavior {
  ...
  ///    so the animation should be paused or stopped when the `NELottieAnimationView` is not visible.
  ...
  public static func `default`(for renderingEngine: NERenderingEngine) -> NELottieBackgroundBehavior {
...
// MARK: - NELottieLoopMode

/// Defines animation loop behavior
public enum NELottieLoopMode: Hashable {
...
extension NELottieLoopMode: Equatable {
  public static func == (lhs: NELottieLoopMode, rhs: NELottieLoopMode) -> Bool {
    switch (lhs, rhs) {
...
// MARK: - NELottieAnimationView

/// A UIView subclass for rendering Lottie animations.
///  - Also available as a SwiftUI view (`NELottieView`) and a CALayer subclass (`NELottieAnimationLayer`)
@IBDesignable
open class NELottieAnimationView: NELottieAnimationViewBase {
  // MARK: Lifecycle
  ...
  public init(animation: NELottieAnimation?,
    ...
    lottieAnimationLayer = NELottieAnimationLayer(
      animation: animation,
  ...
  public init(configuration: NELottieConfiguration = .shared,
              logger: NELottieLogger = .shared) {
    lottieAnimationLayer = NELottieAnimationLayer(configuration: configuration, logger: logger)
    self.logger = logger
    super.init(frame: .zero)
    commonInit()
  }
  ...
  convenience init(animationSource: NELottieAnimationSource?,
                   imageProvider: NEAnimationImageProvider? = nil,
                   textProvider: NEAnimationKeypathTextProvider = NEDefaultTextProvider(),
                   fontProvider: NEAnimationFontProvider = NEDefaultFontProvider(),
                   configuration: NELottieConfiguration = .shared,
  ...
  open func play(_ mode: NELottiePlaybackMode.PlaybackMode, completion: NELottieCompletionBlock? = nil) {
  ...
  public var valueProviders: [NEAnimationKeypath: NEAnyValueProvider] {
  ...
  /// }
  /// ```
  public var animationLoaded: ((_ animationView: NELottieAnimationView, _ animation: NELottieAnimation) -> Void)? {
    didSet {
      if let animation {
  ...
  public var currentPlaybackMode: NELottiePlaybackMode? {
  ...
  /// Logs the result of `allHierarchyKeypaths()` to the `NELottieLogger`.
  ...
  /// let layerKeypath = NEAnimationKeypath(keypath: "Layer 1")
  ///
  /// /// Wrap the custom view in an `NEAnimationSubview`
  /// let subview = NEAnimationSubview()
  /// subview.addSubview(customView)
  ///
  /// /// Set the provider on the animationView.
  /// animationView.addSubview(subview, forLayerAt: layerKeypath)
  /// ```
  public func addSubview(_ subview: NEAnimationSubview, forLayerAt keypath: NEAnimationKeypath) {
  ...
  let lottieAnimationLayer: NELottieAnimationLayer
  ...
  @IBInspectable var animationName: String? {
    didSet {
      lottieAnimationLayer.animation = animationName.flatMap { NELottieAnimation.named($0, animationCache: nil)
      }
    }
  }
