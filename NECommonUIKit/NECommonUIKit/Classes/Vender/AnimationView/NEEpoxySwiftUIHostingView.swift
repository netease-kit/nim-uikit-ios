...
// found in the LICENSE file.
  ...
  // MARK: - NESwiftUIHostingViewReuseBehavior

  /// The reuse behavior of an `NEEpoxySwiftUIHostingView`.
  enum NESwiftUIHostingViewReuseBehavior: Hashable {
    /// Instances of a `NEEpoxySwiftUIHostingView` with `RootView`s of same type can be reused within
    /// the Epoxy container.
    ///
    /// This is the default reuse behavior.
    case reusable
    /// Instances of a `NEEpoxySwiftUIHostingView` with `RootView`s of same type can only reused within
  ...
  extension NECallbackContextEpoxyModeled
    where
    Self: NEWillDisplayProviding & NEDidEndDisplayingProviding,
    CallbackContext: NEViewProviding & NEAnimatedProviding {
    /// Updates the appearance state of a `NEEpoxySwiftUIHostingView` in coordination with the
    ...
    func linkDisplayLifecycle<RootView: View>() -> Self
      where
      CallbackContext.View == NEEpoxySwiftUIHostingView<RootView> {
  ...
  // MARK: - NEEpoxySwiftUIHostingView

  /// A `UIView` that hosts a SwiftUI view within an Epoxy container, e.g. an Epoxy `CollectionView`.
  ///
  /// Wraps an `NEEpoxySwiftUIHostingController` and adds it as a child view controller to the next
  ...
  ///
  /// - SeeAlso: `NEEpoxySwiftUIHostingController`
  @available(iOS 13.0, tvOS 13.0, *)
  final class NEEpoxySwiftUIHostingView<RootView: View>: UIView, NEEpoxyableView {
    // MARK: Lifecycle

    init(style: Style) {
      // Ignore the safe area to ensure the view isn't laid out incorrectly when being sized while
      // overlapping the safe area.
      epoxyContent = NEEpoxyHostingContent(rootView: style.initialContent.rootView)
      viewController = NEEpoxySwiftUIHostingController(
        rootView: .init(content: epoxyContent, environment: epoxyEnvironment),
        ignoreSafeArea: true
      )

      dataID = style.initialContent.dataID ?? NEDefaultDataID.noneProvided as AnyHashable
    ...
    struct Style: Hashable {
      init(reuseBehavior: NESwiftUIHostingViewReuseBehavior, initialContent: Content) {
        self.reuseBehavior = reuseBehavior
        self.initialContent = initialContent
      }

      var reuseBehavior: NESwiftUIHostingViewReuseBehavior
    ...
    func setContent(_ content: Content, animated _: Bool) {
      // This triggers a change in the observed `NEEpoxyHostingContent` object and allows the
      // propagation of the SwiftUI transaction, instead of just replacing the `rootView`.
      epoxyContent.rootView = content.rootView
      dataID = content.dataID ?? NEDefaultDataID.noneProvided as AnyHashable
    ...
    private let viewController: NEEpoxySwiftUIHostingController<NEEpoxyHostingWrapper<RootView>>
    private let epoxyContent: NEEpoxyHostingContent<RootView>
    private let epoxyEnvironment = NEEpoxyHostingEnvironment()
    private var dataID: AnyHashable
    private var state: NEAppearanceState = .disappeared

    /// Updates the appearance state of the `viewController`.
    private func transition(to state: NEAppearanceState) {
      ...
      case (to: .appearing, from: .appearing),
        ...
        NEEpoxyLogger.shared.assertionFailure("Impossible state change from \(self.state) to \(state)")
      }
  ...
  // MARK: - NEAppearanceState

  /// The appearance state of a `NEEpoxySwiftUIHostingController` contained within a
  /// `NEEpoxySwiftUIHostingView`.
  private enum NEAppearanceState: Equatable {
  ...
  // MARK: - NEEpoxyHostingContent

  /// The object that is used to communicate changes in the root view to the
  /// `NEEpoxySwiftUIHostingController`.
  @available(iOS 13.0, tvOS 13.0, *)
  final class NEEpoxyHostingContent<RootView: View>: ObservableObject {
    // MARK: Lifecycle
  ...
  // MARK: - NEEpoxyHostingEnvironment

  /// The object that is used to communicate values to SwiftUI views within an
  /// `NEEpoxySwiftUIHostingController`, e.g. layout margins.
  @available(iOS 13.0, tvOS 13.0, *)
  final class NEEpoxyHostingEnvironment: ObservableObject {
    @Published var layoutMargins = EdgeInsets()
    @Published var intrinsicContentSizeInvalidator = NEEpoxyIntrinsicContentSizeInvalidator(invalidate: {})
  }

  // MARK: - NEEpoxyHostingWrapper

  /// The wrapper view that is used to communicate values to SwiftUI views within an
  /// `NEEpoxySwiftUIHostingController`, e.g. layout margins.
  @available(iOS 13.0, tvOS 13.0, *)
  struct NEEpoxyHostingWrapper<Content: View>: View {
    @ObservedObject var content: NEEpoxyHostingContent<Content>
    @ObservedObject var environment: NEEpoxyHostingEnvironment
