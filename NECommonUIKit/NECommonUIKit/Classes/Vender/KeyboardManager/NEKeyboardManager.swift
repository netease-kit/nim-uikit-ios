...
@objc public final class NEKeyboardManager: NSObject {
  /**
   Returns the default singleton instance.
   */
  @objc public static let shared = NEKeyboardManager()

  /**
   Invalid point value.
   */
  static let kNECGPointInvalid = CGPoint(
  ...
  @objc public var enable = false {
    didSet {
      ...
      if enable, !oldValue {
        // If keyboard is currently showing. Sending a fake notification for keyboardWillHide to
        // retain view's original position.
   ...
   */
  @objc public var toolbarManageBehaviour = NEAutoToolbarManageBehaviour.bySubviews
   ...
   */
  @objc public var previousNextDisplayMode = NEPreviousNextDisplayMode.default
  ...
  @objc @discardableResult public func resignFirstResponder() -> Bool {
    ...
    guard textFieldRetain.resignFirstResponder() else {
      ...
      // If it refuses to resign then becoming it first responder again for getting notifications
      // callback.
  ...
  /// Add TextField/TextView Notifications customised Notifications. For example while using
  // YYTextView https://github.com/ibireme/YYText
  ...
  @objc public func registerTextFieldViewClass(_ aClass: UIView.Type,
    ...
    NotificationCenter.default.addObserver(
      ...
      name: Notification.Name(rawValue: didBeginEditingNotificationName),
      object: nil
    )
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(textFieldViewDidEndEditing(_:)),
      name: Notification.Name(rawValue: didEndEditingNotificationName),
      object: nil
    )
  }

  @objc public func unregisterTextFieldViewClass(_ aClass: UIView.Type,
                                                 didBeginEditingNotificationName: String,
                                                 didEndEditingNotificationName: String) {
  ...
  override init() {
    ...
    // Loading NEToolbar, NETitleBarButtonItem, NEBarButtonItem to fix first time keyboard appearance delay
    // (Bug ID: #550)
    ...
    textField.neAddDoneOnKeyboardWithTarget(nil, action: #selector(doneAction(_:)))
    textField.neAddPreviousNextDoneOnKeyboardWithTarget(
      nil,
      previousAction: #selector(previousAction(_:)),
      nextAction: #selector(nextAction(_:)),
    ...
    toolbarPreviousNextAllowedClasses.append(NEPreviousNextView.self)
  ...
  func keyWindow() -> UIWindow? {
    if let keyWindow = textFieldView?.window {
      ...
      // If original key window is not nil and the cached keywindow is also not original keywindow
      // then changing keywindow.
  ...
  @objc public func reloadLayoutIfNeeded() {
    guard privateIsEnabled(),
          keyboardShowing,
          topViewBeginOrigin.equalTo(NEKeyboardManager.kNECGPointInvalid) == false,
...
@available(iOSApplicationExtension, unavailable)
extension NEKeyboardManager: UIGestureRecognizerDelegate {
  ...
  @objc public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer,
    ...
    //  Should not recognize gesture if the clicked view is either UIControl or UINavigationBar(<Back
    //  button etc...)    (Bug ID: #145)
