...
final class NEColorEffectValue: NEEffectValue {
  // MARK: Lifecycle

  required init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: NECodingKeys.self)
    value = try? container.decode(NEKeyframeGroup<NELottieColor>.self, forKey: .value)
    try super.init(from: decoder)
  }

  required init(dictionary: [String: Any]) throws {
    let valueDictionary: [String: Any] = try dictionary.value(for: NECodingKeys.value)
    value = try NEKeyframeGroup<NELottieColor>(dictionary: valueDictionary)
  ...
  let value: NEKeyframeGroup<NELottieColor>?
