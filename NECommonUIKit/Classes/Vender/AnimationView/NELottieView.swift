...
// found in the LICENSE file.
  ...
  public struct NELottieView<Placeholder: View>: NEUIViewConfiguringSwiftUIView {
    // MARK: Lifecycle

    /// Creates a `LottieView` that displays the given animation
    public init(animation: NELottieAnimation?) where Placeholder == EmptyView {
      _animationSource = State(initialValue: animation.map(NELottieAnimationSource.lottieAnimation))
      placeholder = nil
    }
    ...
    /// }
    /// ```
    public init(dotLottieFile: NEDotLottieFile?) where Placeholder == EmptyView {
      _animationSource = State(initialValue: dotLottieFile.map(NELottieAnimationSource.dotLottieFile))
      placeholder = nil
    }
    ...
    public init(_ loadAnimation: @escaping () async throws -> NELottieAnimation?) where Placeholder == EmptyView {
    ...
    public init(_ loadAnimation: @escaping () async throws -> NELottieAnimation?,
                @ViewBuilder placeholder: @escaping (() -> Placeholder)) {
      self.init {
        try await loadAnimation().map(NELottieAnimationSource.lottieAnimation)
      } placeholder: {
        placeholder()
      }
    }
    ...
    /// }
    /// ```
    public init(_ loadDotLottieFile: @escaping () async throws -> NEDotLottieFile?) where Placeholder == EmptyView {
    ...
    /// }
    /// ```
    public init(_ loadDotLottieFile: @escaping () async throws -> NEDotLottieFile?,
                @ViewBuilder placeholder: @escaping (() -> Placeholder)) {
      self.init {
        try await loadDotLottieFile().map(NELottieAnimationSource.dotLottieFile)
      } placeholder: {
        placeholder()
      }
    }
    ...
    /// If you wish to call `loadAnimation` again at a different time, you can use `.reloadAnimationTrigger(...)`.
    /// While the animation is loading, the `placeholder` view is shown in place of the `LottieAnimationView`.
    public init(_ loadAnimation: @escaping () async throws -> NELottieAnimationSource?) where Placeholder == EmptyView {
      self.init(loadAnimation, placeholder: EmptyView.init)
    }
    ...
    /// If you wish to call `loadAnimation` again at a different time, you can use `.reloadAnimationTrigger(...)`.
    /// While the animation is loading, the `placeholder` view is shown in place of the `LottieAnimationView`.
    public init(_ loadAnimation: @escaping () async throws -> NELottieAnimationSource?,
                @ViewBuilder placeholder: @escaping () -> Placeholder) {
    ...
    public var body: some View {
      NELottieAnimationView.swiftUIView {
        NELottieAnimationView(
      ...
      .neValueChanged(value: reloadAnimationTrigger) { _ in
    ...
    public func configure(_ configure: @escaping (NELottieAnimationView) -> Void) -> Self {
      var copy = self
    ...
    public func play(loopMode: NELottieLoopMode = .playOnce) -> Self {
    ...
    public func play(_ playbackMode: NELottiePlaybackMode) -> Self {
      self.playbackMode(playbackMode)
    }

    /// Returns a copy of this view playing with the given playback mode
    public func playing(_ mode: NELottiePlaybackMode.PlaybackMode) -> Self {
    ...
    public func playing(loopMode: NELottieLoopMode) -> Self {
      playbackMode(.playing(.fromProgress(nil, toProgress: 1, loopMode: loopMode)))
    }
    ...
    public func paused(at state: NELottiePlaybackMode.PausedState = .currentFrame) -> Self {
      playbackMode(.paused(at: state))
    }

    /// Returns a copy of this view using the given `LottiePlaybackMode`
    public func playbackMode(_ playbackMode: NELottiePlaybackMode) -> Self {
      var copy = self
    ...
    public func animationDidLoad(_ animationDidLoad: @escaping (NELottieAnimationSource) -> Void) -> Self {
      var copy = self
    ...
    public func animationDidFinish(_ animationCompletionHandler: NELottieCompletionBlock?) -> Self {
      var copy = self
    ...
    public func backgroundBehavior(_ value: NELottieBackgroundBehavior) -> Self {
      configure { view in
    ...
    public func logger(_ logger: NELottieLogger) -> Self {
      var copy = self
      copy.logger = logger
      return copy
    }
    ...
    public func imageProvider<NEImageProvider: NEAnimationImageProvider & Equatable>(_ imageProvider: NEImageProvider) -> Self {
      var copy = self
      ...
      copy.imageProviderConfiguration = (
        ...
        imageProvidersAreEqual: { untypedLHS, untypedRHS in
          guard
            let lhs = untypedLHS as? NEImageProvider,
            let rhs = untypedRHS as? NEImageProvider
          else { return false }
    ...
    /// The image provider must be `Equatable` to avoid unnecessary state updates / re-renders.
    public func textProvider<NETextProvider: NEAnimationKeypathTextProvider & Equatable>(_ textProvider: NETextProvider) -> Self {
      var copy = self
      copy.textProvider = textProvider

      copy = copy.configure { view in
        if (view.textProvider as? NETextProvider) != textProvider {
    ...
    /// Returns a copy of this view with its image provider updated to the given value.
    /// The image provider must be `Equatable` to avoid unnecessary state updates / re-renders.
    public func fontProvider<NEFontProvider: NEAnimationFontProvider & Equatable>(_ fontProvider: NEFontProvider) -> Self {
      var copy = self
      copy.fontProvider = fontProvider

      copy = configure { view in
        if (view.fontProvider as? NEFontProvider) != fontProvider {
    ...
    public func valueProvider<NEValueProvider: NEAnyValueProvider & Equatable>(_ valueProvider: NEValueProvider,
      ...
      -> Self {
      configure { view in
        if (view.valueProviders[keypath] as? NEValueProvider) != valueProvider {
    ...
    public func currentProgress(_ currentProgress: NEAnimationProgressTime?) -> Self {
    ...
    ///    will continue playing uninterrupted.
    public func currentFrame(_ currentFrame: NEAnimationFrameTime?) -> Self {
    ...
    public func reloadAnimationTrigger(_ value: some Equatable, showPlaceholder: Bool = true) -> Self {
      var copy = self
      copy.reloadAnimationTrigger = NEAnyEquatable(value)
    ...
    public func getRealtimeAnimationProgress(_ realtimeAnimationProgress: Binding<NEAnimationProgressTime>?) -> some View {
    ...
    ///  - If the binding is `nil`, the `TimelineView` will be paused and no updates will occur to the binding.
    @available(iOS 15.0, tvOS 15.0, macOS 12.0, *)
    public func getRealtimeAnimationFrame(_ realtimeAnimationFrame: Binding<NEAnimationFrameTime>?) -> some View {
    ...
    public func dotLottieConfigurationComponents(
      _ dotLottieConfigurationComponents: NEDotLottieConfigurationComponents)
      -> Self {
      var copy = self
    ...
    var configurations = [NESwiftUIView<NELottieAnimationView, Void>.Configuration]()

    // MARK: Private

    @State private var animationSource: NELottieAnimationSource?
    private var playbackMode: NELottiePlaybackMode?
    private var animationSpeed: Double?
    private var reloadAnimationTrigger: NEAnyEquatable?
    private var loadAnimation: (() async throws -> NELottieAnimationSource?)?
    private var animationDidLoad: ((NELottieAnimationSource) -> Void)?
    private var animationCompletionHandler: NELottieCompletionBlock?
    private var showPlaceholderWhileReloading = false
    private var textProvider: NEAnimationKeypathTextProvider = NEDefaultTextProvider()
    private var fontProvider: NEAnimationFontProvider = NEDefaultFontProvider()
    private var configuration: NELottieConfiguration = .shared
    private var dotLottieConfigurationComponents: NEDotLottieConfigurationComponents = .imageProvider
    private var logger: NELottieLogger = .shared
    private var sizing = NESwiftUIMeasurementContainerStrategy.automatic
    private let placeholder: (() -> Placeholder)?

    private var imageProviderConfiguration: (
      imageProvider: NEAnimationImageProvider,
      imageProvidersAreEqual: (NEAnimationImageProvider, NEAnimationImageProvider) -> Bool
    )?
    ...
    private func applyCurrentAnimationConfiguration(to view: NELottieAnimationView) {
      ...
      if let dotLottieConfiguration = animationSource.dotLottieAnimation?.configuration {
        ...
        if
          ...
          imageProviderConfiguration = (
            ...
            imageProvidersAreEqual: { untypedLHS, untypedRHS in
              guard
                let lhs = untypedLHS as? NEDotLottieImageProvider,
                let rhs = untypedRHS as? NEDotLottieImageProvider
              else { return false }
