...
@objcMembers
open class NEAtMessageManager: NSObject, NE2IMKitClientListener, NEChatListener {
  ...
  private func startFilterRoamingMessagesTaskInWorkqueue() {
    var conversations = [V2NIM2Conversation]()
    weak var weakSelf = self
  ...
  private func removeRecord(message: V2NIM2Message, record: inout [String: AtMEMessageRecord]) -> Bool {
    var didRemove = false
  ...
  /// - Parameter message: 消息
  /// - Parameter record: at 消息记录缓存
  private func addAtRecordWithCompare(message: V2NIM2Message, record: inout [String: AtMEMessageRecord]) {
  ...
  /// - Parameter message: 消息
  /// - Parameter record: at 消息记录缓存
  private func addAtRecord(message: V2NIM2Message, record: inout [String: AtMEMessageRecord]) {
    if let atMeRecord = record[message.conversationId ?? ""] {
      let lastTime = atMeRecord.lastTime?.doubleValue ?? 0
      if lastTime < message.createTime {
        let atMessage = AtMessageModel()
  ...
  private func getAllConversation(_ conversations: inout [V2NIM2Conversation], _ offset: Int64 = 0, _ completion: @escaping (NSError?) -> Void) {
    let limit = 20
    var temConversations = conversations
    NE2ConversationProvider.shared.getConversationList(offset, limit) { [weak self] result, error in
      if let err = error {
        completion(err)
      } else {
  ...
  /// - Parameter record: at 消息记录缓存
  private func removeRecordWithMessageref(messageRefer: V2NIM2MessageRefer, record: inout [String: AtMEMessageRecord]) -> Bool {
    var didRemove = false
