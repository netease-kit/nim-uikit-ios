    ...
    func requestData(){
        ...
        viewModel.fetchServerSessions(option: params) { error, recentSessions in
            if error == nil {
                ...
                NELog.errorLog(self.className, desc: "❌fetchServerSessions failed，error = \(error!)")
            }
        }
    }
...
extension ConversationListViewController{
    ...
    private func onTopRecentAtIndexPath(rencent:NIMRecentSession,indexPath:IndexPath,isTop:Bool,_ completion:@escaping (NSError?,NIMStickTopSessionInfo?)->()){
        ...
        if isTop {
            ...
            viewModel.removeStickTopSession(params: params) { error, topSessionInfo in
                if let err = error {
                    NELog.errorLog(weakSelf?.className ?? "ConversationListViewController", desc: "❌removeStickTopSession failed，error = \(err)")
            ...
            viewModel.addStickTopSession(session: session, { error, newInfo in
                if let err = error {
                    NELog.errorLog(weakSelf?.className ?? "ConversationListViewController", desc: "❌addStickTopSession failed，error = \(err)")
                    completion(error as NSError?,nil)
                    return
                }else {
...
extension ConversationListViewController:UITableViewDelegate,UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let deleteAction = UITableViewRowAction.init(style: .destructive, title: "删除") { action, indexPath in
            ...
            weakSelf?.didDeleteConversationCell(model: conversationModel ?? ConversationListModel(), indexPath: indexPath)
       ...
       //置顶和取消置顶
        ...
        let topAction = UITableViewRowAction.init(style: .destructive, title: isTop ? "取消置顶":"置顶") { action, indexPath in
            if let recentSesstion = conversationModel?.recentSession {
                weakSelf?.onTopRecentAtIndexPath(rencent: recentSesstion, indexPath: indexPath, isTop: isTop, { error, sessionInfo in
                    if error == nil {
                        if isTop {
                            weakSelf?.didRemoveStickTopSession(model: conversationModel ?? ConversationListModel(), indexPath: indexPath)
                        }else {
                            weakSelf?.didAddStickTopSession(model: conversationModel ?? ConversationListModel(), indexPath: indexPath)
                        }
                    }
                })
            }
...
extension ConversationListViewController {
    ...
    ///   - sessionId: 会话id
    ///   - indexPath: indexpath
    open func onselectedTableRow(sessionType:NIMSessionType,sessionId:String,indexPath:IndexPath) {
        if sessionType == .P2P {
