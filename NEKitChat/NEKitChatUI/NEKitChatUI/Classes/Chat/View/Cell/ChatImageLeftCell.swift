...
class ChatImageLeftCell: ChatBaseLeftCell {
    ...
    func commonUI() {
        ...
        contentImageView.addCustomCorner(conrners: [.bottomLeft, .bottomRight, .topRight], radius: 8, backcolor: .white)
    ...
    override func setModel(_ model: MessageContentModel) {
        ...
        if let m = model as? MessageImageModel,let url = m.imageUrl {
            contentImageView.sd_setImage(with: URL.init(string: url), placeholderImage: nil, options: .retryFailed, progress: nil, completed: nil)
        }else {
