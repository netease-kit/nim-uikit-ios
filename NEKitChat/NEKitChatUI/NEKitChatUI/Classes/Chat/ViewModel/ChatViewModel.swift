...
public class ChatViewModel: NSObject, ChatRepoMessageDelegate, NIMChatManagerDelegate, NIMConversationManagerDelegate, NIMSystemNotificationManagerDelegate,ChatExtendProviderDelegate {
    
    public var session: NIMSession
    public var messages: [MessageModel] = [MessageModel]()
    ...
    public func sendTextMessage(text: String, _ completion: @escaping (Error?) -> Void) {
        ...
        repo.sendMessage(message: MessageUtils.textMessage(text: text), session: self.session, completion)
    }
    
    public func sendAudioMessage(filePath: String, _ completion: @escaping (Error?) -> Void) {
        repo.sendMessage(message: MessageUtils.audioMessage(filePath: filePath), session: self.session, completion)
    }
    
    public func sendImageMessage(image: UIImage, _ completion: @escaping (Error?) -> Void) {
        // TODO:
        repo.sendMessage(message: MessageUtils.imageMessage(image: image), session: session, completion)
    }
    ...
    public func sendVideoMessage(url: URL, _ completion: @escaping (Error?) -> Void){
        ...
        VideoFormatConvert.convertToMP4(with: url) { path, image in
            if let p = path, let s = weakSelf?.session {
                ...
                QChatLog.errorLog("chat veiw model", desc: "convert mov to mp4 failed")
            }
        }
    }
...
//                mark read
    ...
    public func getMoreMessageHistory(_ completion: @escaping (Error?,NSInteger, [MessageModel]?) -> Void) {
        ...
        ChatProvider.shared.getMessageHistory(session: self.session, message: self.oldMsg, limit: messagPageNum) { [weak self] error, messages in
            if let messageArray = messages, messageArray.count > 0 {
                ...
                let isCredible = weakSelf?.isMessageCredible(message: messageArray.first ?? NIMMessage())
                ...
                if let messageArray = messages,messageArray.isEmpty,weakSelf?.credibleTimestamp ?? 0 > 0 {
            ...
            if error == nil {
                if let messageArray = messages, messageArray.count > 0 {
                    ...
                    if let updateMessage = messageArray.first,updateCredible {
                        ...
                        weakSelf?.repo.updateIncompleteSessions(messages: [updateMessage], { error, recentSessions in
                            if error != nil {
                                QChatLog.errorLog(weakSelf?.className ?? "ChatViewModel", desc: "❌updateIncompleteSessions failed，error = \(error!)")
                            }
                        })
                    }
    ...
    //下拉获取历史消息
    public func dropDownRemoteRefresh( _ completion: @escaping (Error?,NSInteger,[MessageModel]?) -> Void){
        ...
        if isCredible {//继续拉去本地消息
            ...
            //不可信拉去远端消息
            getRemoteHistoryMessage(direction:.old,updateCredible:false, option: option, completion)
        }
        
    }
    
    //上拉获取最新消息
    public func pullRemoteRefresh(_ completion: @escaping (Error?,NSInteger, [MessageModel]?) -> Void){
        let option = NIMHistoryMessageSearchOption()
        ...
        if isCredible {
            if anchor != nil {
                //搜索历史记录进入
                searchMessageHistory(direction:.new,startTime: newMsg?.timestamp ?? 0, endTime: 0, completion)
            }
            
        }else {
            getRemoteHistoryMessage(direction:.new,updateCredible:false, option: option, completion)
            
        }
    }
 
    //搜索历史记录查询的本地消息
    public func searchMessageHistory(direction:LoadMessageDirection,startTime:TimeInterval,endTime:TimeInterval, _ completion: @escaping (Error?, NSInteger,[MessageModel]?) -> Void) {
...
//        mark session read
    ...
    private func markReadInTeam(messages: [NIMMessage], _ completion: @escaping (Error?) -> Void) {
        ...
        for message in messages {
            ...
            if receiptEnable, !message.isTeamReceiptSended {
                let receipt = NIMMessageReceipt.init(message: message)
    ...
    public func revokeMessage(message: NIMMessage, _ completion: @escaping (Error?) -> ())  {
    ...
    public func onReceive(_ notification: NIMCustomSystemNotification) {
        ...
        if let content = notification.content, let dic = getDictionaryFromJSONString(content) as? [String : Any], let typing = dic["typing"] as? Int {
            if typing == 1 {
...
//    MARK: ChatExtendProviderDelegate
    public func onNotifyAddMessagePin(pinItem: NIMMessagePinItem) {
        ...
        for (i, model) in self.messages.enumerated() {
            if pinItem.messageServerID == model.message?.serverID {
                ...
                self.messages[i].pinAccount = pinID
                self.messages[i].pinShowName = self.getShowName(userId: pinID, teamId: session.sessionId)
                index = i
                break
            }
        }
        if index >= 0, let msg = self.messages[index].message {
            self.delegate?.onAddMessagePin(msg, atIndexs: [IndexPath(row: index, section: 0)])
        }
    }
    ...
    public func onNotifyRemoveMessagePin(pinItem: NIMMessagePinItem) {
        ...
        for (i, model) in self.messages.enumerated() {
            if pinItem.messageServerID == model.message?.serverID {
                self.messages[i].isPined = false
                self.messages[i].pinAccount = nil
                self.messages[i].pinShowName = nil
                index = i
                break
            }
        }
        if index >= 0, let msg = self.messages[index].message {
            self.delegate?.onRemoveMessagePin(msg, atIndexs: [IndexPath(row: index, section: 0)])
        }
    }
...
//    MARK: collection
    func addColletion(_ message: NIMMessage, completion: @escaping (NSError?, NIMCollectInfo?) -> ()) {
...
//    MARK: revoke
    public func onRecvRevokeMessageNotification(_ notification: NIMRevokeMessageNotification) {
        ...
        self.revokeMessageUpdateUI(msg)

    }
    
    public func onRecvMessageReceipts(_ receipts: [NIMMessageReceipt]) {
        print("chatViewModel: receipts:\(receipts.count) messageId:\(receipts.first?.messageId) messageId:\(receipts.first?.timestamp)")
        self.delegate?.didReadedMessageIndexs()
    ...
    public func avalibleOperationsForMessage(_ model: MessageContentModel?) -> [OperationItem]? {
        ...
        case .text:
            ...
            items = [OperationItem.copyItem(),OperationItem.replayItem(),OperationItem.forwardItem(),pinItem,OperationItem.selectItem(),OperationItem.collectionItem(),OperationItem.deleteItem()]
            
        case .image, .video:
            if let isPin = model?.isPined, isPin {
                pinItem = OperationItem.removePinItem()
            }
            items = [OperationItem.replayItem(),OperationItem.forwardItem(),pinItem,OperationItem.selectItem(),OperationItem.collectionItem(),OperationItem.deleteItem()]
        case .audio:
            if let isPin = model?.isPined, isPin {
                pinItem = OperationItem.removePinItem()
            }
            items = [OperationItem.replayItem(),pinItem,OperationItem.selectItem(),OperationItem.collectionItem(),OperationItem.deleteItem()]
...
//      find messages that need to update UI
        ...
        for model in self.messages.reversed() {
...
//                        <#code#>
        ...
        if let uid = message.from {
            let user = self.getUserInfo(userId: uid)
            var fullName = uid
            ...
            if session.sessionType == .team {
                // team
                let teamMember = self.getTeamMember(userId: uid, teamId: session.sessionId)
        ...
        if let pin = repo.pinItem(message) {
            ...
            model.pinShowName = self.getShowName(userId: pinID, teamId: session.sessionId)
        }else {
            model.isPined = false
        }
        return model
    ...
    private func getReplyMessage(message: NIMMessage) -> MessageModel? {
        ...
        if let message = ConversationProvider.shared.messagesInSession(session, messageIds: [id])?.first {
    ...
    private func getUserInfo(_ userId: String,_ completion: @escaping (User?, NSError?)->()) {
        if let user = userInfo[userId] {
            completion(user, nil)
        }
        if let user = FriendProvider.shared.getUserInfo(userId: userId)  {
            userInfo[userId] = user
            completion(user, nil)
        }
        
        FriendProvider.shared.fetchUserInfo(accountList: [userId]) {[weak self] users, error in
...
//    获取展示的用户名字，p2p： 备注》昵称>ID  team: 备注〉群昵称》 昵称〉 ID
    private func getShowName(userId: String, teamId: String?) -> String {
        let user = self.getUserInfo(userId: userId)
        var fullName = userId
        if let nickName = user?.userInfo?.nickName {
            fullName = nickName
        }
//        model.avatar = user?.userInfo?.thumbAvatarUrl
        if let tID = teamId, session.sessionType == .team {
            // team
            let teamMember = self.getTeamMember(userId: userId, teamId:tID)
            if let teamNickname = teamMember?.nickname {
                fullName = teamNickname
            }
        }
...
//    全名后几位
    private func getShortName(name: String, length: Int) -> String {
        return name.count > length ? String(name[name.index(name.endIndex, offsetBy: -length)...]) : name
    }
...
//            remove time tip
            let last = index - 1
            if last >= 0,let timeModel = self.messages[last] as? MessageTipsModel, timeModel.type == .time {
                self.messages.removeSubrange(last...index)
        ...
        self.delegate?.onDeleteMessage(message, atIndexs: indexs)
    }
...
//            let model = MessageRevokeModel(message: message)
            ...
            if var messages = self?.messages {
                for (i, model) in messages.enumerated() {
                    if message.messageId == model.message?.messageId {
                        ...
                        messages[i].pinShowName = self?.getShowName(userId: NIMSDK.shared().loginManager.currentAccount(), teamId: message.session?.sessionId)
    ...
    public func removePinMessage(_ message: NIMMessage, _ completion: @escaping (Error?, NIMMessagePinItem?, Int)->()) {
        let item = NIMMessagePinItem(message: message)
