...
public class NotificationMessageUtils {
    public class func textForNotification(message: NIMMessage) -> String {
        ...
        if let object = message.messageObject as? NIMNotificationObject {
            ...
            case .team:
                return self.textForTeamNotificationMessage(message: message)
            case .superTeam:
                return ""
            case .netCall:
                return ""
    ...
    public class func textForTeamNotificationMessage(message: NIMMessage) -> String {
        ...
        if let object = message.messageObject as? NIMNotificationObject {
            if let content = object.content as? NIMTeamNotificationContent {
                let fromName = self.fromName(message: message)
                let toNames = self.toName(message: message)
                let toFirstName = toNames.first ?? ""
                let teamName = self.teamName(message: message)
                ...
                case .update:
                     text = textOfUpdateTeam(fromName: fromName, teamName: teamName, content: content)
                case .leave:
            ...
            if let content = object.content as? NIMTeamNotificationContent {
                if content.sourceID == NIMSDK.shared().loginManager.currentAccount() {
                    ...
                    if let sourceId = content.sourceID  {
                        let user = FriendProvider.shared.getUserInfo(userId: sourceId)
    ...
    public class func toName(message: NIMMessage) -> [String] {
        var toNames = [String]()
        guard let object = message.messageObject as? NIMNotificationObject, let content = object.content as? NIMTeamNotificationContent,let targetIDs = content.targetIDs  else {
            return toNames
        }
        for targetID in targetIDs {
            if targetID == NIMSDK.shared().loginManager.currentAccount() {
                ...
                toNames.append(FriendProvider.shared.getUserInfo(userId: targetID)?.showName() ?? "")
            }
        }
        return toNames
    }
    ...
    private class func textOfUpdateTeam(fromName:String, teamName: String, content: NIMTeamNotificationContent) -> String {
        ...
        if attach.values?.count == 1 {
            ...
            switch tag {
            case 3:
                text = fromName + localizable("has_updated") + teamName + " " + localizable("team_name")
            case 14:
                text = fromName + localizable("has_updated") + teamName + " " + localizable("team_intro")
            case 15:
                text = fromName + localizable("has_updated") + teamName + " " + localizable("team_anouncement")
            case 16:
                text = fromName + localizable("has_updated") + teamName + " " + localizable("team_join_mode")
            case 18:
                text = fromName + localizable("has_updated") + " " + localizable("team_custom_info")
            case 19:
                text = fromName + localizable("has_updated") + " " + localizable("team_custom_info")
            case 20:
                text = fromName + localizable("has_updated") + teamName + " " + localizable("team_avatar")
            case 21:
                text = fromName + localizable("has_updated") + " " + localizable("team_be_invited_author")
            case 22:
                text = fromName + localizable("has_updated") + " " + localizable("team_be_invited_permission")
            case 23:
                text = fromName + localizable("has_updated") + " " + localizable("team_update_info_permission")
            case 24:
                text = fromName + localizable("has_updated") + " " + localizable("team_update_client_custom")
            case 100:
