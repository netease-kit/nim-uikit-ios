...
public class NEEmotionTool: NSObject {
    class func getAttWithStr(str:String,font:UIFont, _ offset: CGPoint = CGPoint(x: 0, y: -3) )->NSMutableAttributedString {
        ...
        //找出所有符合正则的位置集合
        let regularArr = reExpression?.matches(in: str, options: .reportProgress, range: NSRange.init(location: 0, length: str.count))
            ...
            for i in (0...regArr.count - 1).reversed() {
                ...
                for (idx,obj) in targetEmotions.enumerated() {
                    ...
                    if ocStr.substring(with: result.range) == obj.tag {
                        attStr.replaceCharacters(in: result.range, with: getAttWithEmotion(emotion: obj, font: font, offset: offset))
                        break
                    }
                }
            }
    ...
    class func getAttWithEmotion(emotion:NIMInputEmoticon,font:UIFont,offset: CGPoint) ->NSAttributedString {
        let textAttachment = NEEmotionAttachment()
        textAttachment.emotion = emotion
        let height = font.lineHeight
        textAttachment.bounds = CGRect.init(x: offset.x, y: offset.y, width: height, height: height)
        return NSAttributedString.init(attachment: textAttachment)
    }
}
