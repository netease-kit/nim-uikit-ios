...
public class NIMInputEmoticonLayout:NSObject {
    ...
    public init(width:CGFloat) {
        ...
        columes = ((Int(width) - NIMKit_EmojiLeftMargin - NIMKit_EmojiRightMargin) / Int(NIMKit_EmojImageWidth))
        itemCountInPage = rows * columes - 1
        cellWidth = CGFloat((Int(width) - NIMKit_EmojiLeftMargin - NIMKit_EmojiRightMargin) / columes)
...
public class NIMInputEmoticonManager: NSObject {
        ...
        if let path = filePath {
            ...
            array?.forEach({ dict in
                if let convertDict = (dict as? NSDictionary) {
                    let info = convertDict["info"] as? Dictionary<String,Any>
                    let emotions = convertDict["data"] as? NSArray
                    let cataLog = catalogByInfo(info: info as NSDictionary?, emoticonsArray: emotions)
    ...
    func catalogByInfo(info:NSDictionary?,emoticonsArray:NSArray?) -> NIMInputEmoticonCatalog {
        ...
        let cataLog = NIMInputEmoticonCatalog.init()
        
        guard let infoDict = info ,let emotions = emoticonsArray else {
            QChatLog.errorLog(classTag, desc: "❌info or emoticonsArray is nil")
            return cataLog
        }
        ...
        emotions.forEach { emoticonDict in
            if let dict = (emoticonDict as? NSDictionary ){
                let emotion = NIMInputEmoticon.init()
    ...
    public func emoticonByTag(tag:String) ->NIMInputEmoticon? {
        ...
        guard let clogs = catalogs else {
            QChatLog.errorLog(classTag, desc: "❌catalogs is nil")
            return emotion
        }
