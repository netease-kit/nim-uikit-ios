...
public class InputEmoticonTabView: UIControl {
    ...
    func setUpSubViews(){
        ...
        NSLayoutConstraint.activate([
            ...
            sendButton.rightAnchor.constraint(equalTo: self.rightAnchor),
    ...
    public func loadCatalogs(_ emoticonCatalogs: [NIMInputEmoticonCatalog]?) {
        ...
        guard let catalogs = emoticonCatalogs else {
            QChatLog.errorLog(className, desc: "❌emoticonCatalogs is nil")
            return
        }
        catalogs.forEach { catelog in
            ...
            let sep = UIView.init(frame: CGRect.init(x: 0, y: 0, width: 0.5, height: 35))
