...
public class ChatViewController: NEBaseViewController, UINavigationControllerDelegate, ChatInputViewDelegate, ChatViewModelDelegate, NIMMediaManagerDelegate,MessageOperationViewDelegate, UIGestureRecognizerDelegate {
    ...
    public func getSessionInfo(session: NIMSession) {}
...
//                if let err = error {
//                    QChatLog.errorLog(self?.tag ?? "ChatViewController", desc: "❌getMessageHistory error, error:\(err)")
//                }else {
//                    if let tempArray = weakSelf?.viewmodel.messages,tempArray.count > 0 {
//                        weakSelf?.tableView.reloadData()
//                        weakSelf?.tableView.scrollToRow(at: IndexPath(row: tempArray.count - 1, section: 0), at: .bottom, animated: false)
...
//            QChatLog.infoLog("send video message", desc: error?.localizedDescription ?? "no error")
            weak var weakSelf = self
            viewmodel.sendVideoMessage(url: url) { error in
                
                if let err = error {
                    QChatLog.errorLog("send video message", desc: err.localizedDescription)
                    weakSelf?.showToast(err.localizedDescription)
                }
            }
            return
        ...
        viewmodel.sendImageMessage(image: image, {[weak self] error in
            QChatLog.infoLog("send image message", desc: error?.localizedDescription ?? "no error")
            if error != nil {
                self?.view.makeToast(error?.localizedDescription)
            }else {
