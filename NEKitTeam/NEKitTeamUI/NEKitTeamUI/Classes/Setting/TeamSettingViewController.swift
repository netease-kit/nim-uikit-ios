...
public class TeamSettingViewController: NEBaseViewController {
    ...
    public var cellClassDic = [SettingCellType.SettingArrowCell.rawValue: TeamArrowSettingCell.self, SettingCellType.SettingSwitchCell.rawValue: TeamSettingSwitchCell.self, SettingCellType.SettingSelectCell.rawValue: TeamSettingSelectCell.self]
    ...
    lazy var contentTable: UITableView = {
        ...
        table.tableFooterView = UIView(frame: CGRect(x: 0,y: 0,width: view.frame.size.width,height: 12))
    ...
    func getHeaderView() -> UIView {
        ...
        if viewmodel.isNormalTeam() == false , viewmodel.isOwner() == false , let inviteMode = viewmodel.teamInfoModel?.team?.inviteMode, inviteMode == .manager {
    ...
    func setupUserInfoCollection(_ cornerView: UIView) {
        ...
        NSLayoutConstraint.activate([
            ...
            userinfoCollection.rightAnchor.constraint(equalTo: cornerView.rightAnchor, constant: -15),
            userinfoCollection.heightAnchor.constraint(equalToConstant: 32)
        ])
        
        userinfoCollection.register(TeamUserCell.self, forCellWithReuseIdentifier: "\(TeamUserCell.self)")
